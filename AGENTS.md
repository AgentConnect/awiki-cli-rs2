...
- 功能完成前必须审查 `../awiki-system-test` 中对应的 suite/case catalog，确认成功路径、相关失败路径、跨服务行为、持久化清理和回归边界是否完整；覆盖不足时必须在同一任务跨仓补齐，不适用时记录具体原因。实际运行时使用 `AWIKI_SYSTEM_TEST_MODE=remote` 并通过 `AWIKI_SYSTEM_TEST_TARGET` 显式选择该任务批准的远程环境，不得依赖隐式默认域名；运行 affected/focused case，只有任务或 release gate 明确要求时才扩大到完整 suite。必须记录实际命令、目标环境、通过/失败/跳过数量、失败或跳过原因以及关键环境配置。
...
- 任何生产行为的新增或修改，都必须在同一任务中新增或修改对应单元测试；若现有测试已经精确覆盖该行为及失败分支，必须指出具体测试并记录结果。产品 E2E 使用闭集所有权：AWiki Me App E2E 由 `../awiki-me/tests/e2e/` 拥有，DSH 插件 Web E2E 由 `../dsh-awiki/tests/e2e/` 拥有；本仓不新增或强制要求仓库内产品 E2E。
