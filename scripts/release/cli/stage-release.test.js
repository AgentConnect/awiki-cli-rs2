...
test('stages a complete self-hosted package, manifest, Skill, and onboarding snapshot', () => {
  ...
  try {
    ...
    writeArtifacts(artifacts, '1.0.17-beta.2');
    ...
    const result = run(process.execPath, [
      ...
      '--artifacts', artifacts,
      '--output', output,
      '--source-tag', 'cli-v1.0.17-beta.2',
      '--source-commit', 'a'.repeat(40),
    ], { cwd: rootDir });
    assert.equal(result.status, 0, result.stderr || result.stdout);

    const manifest = JSON.parse(fs.readFileSync(path.join(output, 'manifest.json'), 'utf8'));
    assert.equal(manifest.latest, '1.0.17-beta.2');
...
test('rejects missing and unexpected workflow artifacts', () => {
  ...
  try {
    ...
    writeServerConfig(serverConfig, temp);
    writeArtifacts(artifacts, '1.0.17-beta.2');
    fs.rmSync(path.join(artifacts, 'awiki-cli-1.0.17-beta.2-linux-amd64.tar.gz'));
    ...
    const baseArgs = [
      ...
      '--source-tag', 'cli-v1.0.17-beta.2', '--source-commit', 'a'.repeat(40),
    ];
