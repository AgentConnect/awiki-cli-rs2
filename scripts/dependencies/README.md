...
用户于 2026-09-24 决定不因提交、更新或合并 PR 自动执行 Dependency modes、Node CI
或 Tier 1 工作流。需要 CI 证据时由用户明确指定并通过 `workflow_dispatch` 手动启动；
未运行的检查不能记为通过，也不使用 `pull_request_target` 执行不受信任源码。
