...
pub use crate::runtime::prompt_context::{allowed_actions_for_task, sender_trust_level_for_task};
use crate::runtime::prompt_context::{
    render_invocation_context_prompt as render_shared_invocation_context_prompt,
    RuntimePromptContext,
};
use crate::runtime::{
    RuntimeInstallStatus, RuntimeInvocationAuthority, RuntimeLaunchContext, RuntimeLaunchOutcome,
...
pub fn render_invocation_context_prompt(invocation: &GenericCliInvocation) -> String {
    let context = RuntimePromptContext {
        controller_did: invocation.context.controller_did.clone(),
        sender_did: invocation.context.sender_did.clone(),
        requester_did: invocation.context.requester_did.clone(),
        requester_full_handle: invocation.context.requester_full_handle.clone(),
        trigger_kind: invocation.context.trigger_kind.clone(),
        conversation_scope_kind: invocation.context.conversation_scope_kind.clone(),
        conversation_scope_key: invocation.context.conversation_scope_key.clone(),
        invocation_authority: invocation.context.invocation_authority.clone(),
        controller_verified: invocation.context.controller_verified,
        sender_trust_level: invocation.context.sender_trust_level.clone(),
        allowed_actions: invocation.context.allowed_actions.clone(),
    };
    render_shared_invocation_context_prompt(&context, &invocation.preferred_language)
}
