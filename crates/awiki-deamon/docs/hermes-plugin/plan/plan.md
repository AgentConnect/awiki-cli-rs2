...
恢复位置：Step 06，状态 `review`，已完成 Hermes session 持久化与 resume/reset 实现和验证，准备提交
...
| 06 | Hermes session 持久化与 resume/reset | 03, 04 | `hermes_native_sessions`、可选通用 session mapping、resume/reset/cleanup | [steps/06-session-persistence.md](steps/06-session-persistence.md) | 必须 | review |
