...
| 2026-06-05 | Runtime Agent 外发消息、附件和 controller final 统一收敛为普通消息；Skill 不暴露加密发送参数，`msg.send` 拒绝非普通消息 security | 当前联调阶段只支持普通消息，避免 E2EE peer/prekey 状态导致消息丢失或失败 | 05、07、08 | 是 |
