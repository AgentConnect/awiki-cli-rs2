...
// A fresh bootstrap authorizes the new device generation. In the same ordinary
// stream it does not acknowledge events between our received cursor and its tail.
pub(super) fn retain_bootstrap_receive_position(
    connection: &Connection,
    ...
    if binding.account_id != input.binding.account_id
        || binding.protocol_device_id != input.binding.protocol_device_id
        || binding.current_did != input.binding.current_did
    ...
    if previous.stream_epoch == input.state.stream_epoch {
        input.state.scan_seq = previous.scan_seq;
    }
    // Lane epochs remain independently authorized. Within an unchanged epoch,
