...
async fn orphan_terminal_notice_does_not_block_new_join_requests() {
    ...
    let started = candidate
        ...
        .start(
            crate::identity::DeviceJoinStartRequest {
                operation_id: "start-orphan-terminal-test".into(),
                did,
                ttl_seconds: 300,
            },
            &document,
    ...
    for (event_id, session, kind, state, payload) in [
        (
            "event-orphan-expired",
            "join-expired-history".to_owned(),
        ...
        (
            "event-new-request",
            started.session.join_session_id.clone(),
            "awiki.device.join-requested.v1",
            "pending",
            json!({"join_request":started.join_request}),
        ),
    ] {
        let notification_value = json!({
        ...
        let meta = crate::internal::system_notification::wire::DirectMeta {
            anp_version: Some("2.0".to_owned()),
            profile: crate::internal::system_notification::wire::DIRECT_PROFILE.to_owned(),
            security_profile: crate::internal::system_notification::wire::TRANSPORT_SECURITY
                .to_owned(),
            sender_did: "did:wba:example.test:service".to_owned(),
            target: crate::internal::system_notification::wire::DirectTarget {
                kind: "did".to_owned(),
    ...
    let stored = client
        .list_verified_device_join_notifications(true)
        .await
        .unwrap();
    assert_eq!(
        stored.len(),
        2,
        "historical terminal evidence remains stored"
    );
}
