...
#[test]
fn management_ready_root_resave_preserves_versioned_access_token() {
    let scenario = scenario();
    let imported = import_receiver_root(&scenario);
    let store = IdentityStore::new(&scenario.receiver.paths);
    store
...
fn import_fails_closed_for_wrong_direction_role_expiry_current_document_and_root() {
    ...
    device_drift_registry.checkpoint.document_hash =
        crate::internal::identity_wire::document::document_hash(&device_drift_document).unwrap();
    assert_eq!(
        scenario
            .import(
                &prepared,
    ...
    root_drift_registry.checkpoint.document_hash =
        crate::internal::identity_wire::document::document_hash(&root_drift_document).unwrap();
    assert_eq!(
        scenario
            .import(
                &prepared,
