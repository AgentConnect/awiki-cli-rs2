...
CREATE INDEX IF NOT EXISTS idx_contacts_owner_identity
...
CREATE INDEX IF NOT EXISTS idx_contacts_owner_identity_source_group
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_contact_handle_bindings_owner_identity_handle_current_unique
    ON contact_handle_bindings(owner_identity_id, handle) WHERE owner_identity_id IS NOT NULL AND is_current = 1;
...
CREATE INDEX IF NOT EXISTS idx_contact_handle_bindings_owner_identity_did
    ON contact_handle_bindings(owner_identity_id, did, last_seen_at DESC);
...
"#,
    ...
    ensure_column(connection, "contacts", "owner_identity_id", "TEXT")?;
    ensure_column(
        connection,
        "contacts",
        "credential_name",
...
pub(crate) fn get_contact_by_did(
    ...
    query_one_contact(
        connection,
        &format!(
            "SELECT * FROM contacts WHERE {} AND did = ?3",
            owner_predicate(1, 2)
        ),
        &[
...
fn get_current_contact_by_handle_for_owner(
    ...
    let from_contact = query_one_contact(
        connection,
        &format!(
            "SELECT * FROM contacts WHERE {} AND handle = ?3",
            owner_predicate(1, 2)
        ),
        &[&owner_identity_id, &owner_did, &handle],
    );
...
WHERE (contact_handle_bindings.owner_identity_id = ?1 OR ((contact_handle_bindings.owner_identity_id IS NULL OR TRIM(contact_handle_bindings.owner_identity_id) = '') AND contact_handle_bindings.owner_did = ?2))
  AND contact_handle_bindings.handle = ?3
...
pub(crate) fn resolve_contact_handle_by_did_for_owner_identity(
    ...
    let contact_handle = connection
        .query_row(
            &format!(
                "SELECT handle FROM contacts WHERE {} AND did = ?3 AND TRIM(COALESCE(handle, '')) <> ''",
                owner_predicate(1, 2)
            ),
            params![owner_identity_id.as_str(), owner_did.as_str(), did.as_str()],
            |row| row.get::<_, Option<String>>(0),
        )
        .optional()
        .map_err(super::local_state_unavailable)?
...
ORDER BY is_current DESC, last_seen_at DESC"#,
    ...
    let did = connection
        .query_row(
            &format!(
                "SELECT did FROM contacts WHERE {} AND handle = ?3",
                owner_predicate(1, 2)
            ),
            params![
                owner_identity_id.as_str(),
...
pub(crate) fn upsert_contact(
    ...
    let existing_by_did = query_contact_did_handle(
        &transaction,
        &format!(
            "SELECT did, handle FROM contacts WHERE {} AND did = ?3",
            owner_predicate(1, 2)
        ),
        &owner_identity_id,
        &owner_did,
    ...
    let existing_by_handle = if handle.is_empty() {
        ...
        query_contact_did_handle(
            &transaction,
            &format!(
                "SELECT did, handle FROM contacts WHERE {} AND handle = ?3",
                owner_predicate(1, 2)
            ),
            &owner_identity_id,
            &owner_did,
    ...
    if !handle.is_empty() && !existing_by_handle.is_empty() && existing_by_handle[0].0.trim() != did
    {
        transaction
            .execute(
                &format!(
                    "UPDATE contacts SET handle = NULL, last_seen_at = ?1 WHERE {} AND did = ?4",
                    owner_predicate(2, 3)
                ),
                params![
                    now.as_str(),
...
fn query_contact_did_handle(
    ...
    let mut statement = connection
        ...
        .map_err(super::local_state_unavailable)?;
    let rows = statement
        .query_map(params![owner_identity_id, owner_did, value], |row| {
            Ok((
...
fn owner_predicate(identity_param: usize, did_param: usize) -> String {
    format!(
        "(owner_identity_id = ?{identity_param} OR ((owner_identity_id IS NULL OR TRIM(owner_identity_id) = '') AND owner_did = ?{did_param}))"
    )
}
...
mod tests {
    ...
    fn contacts_upsert_rebinds_current_handle_and_preserves_history() {
        ...
        let current = get_current_contact_by_handle(&db, "default", "did:owner", "alice").unwrap();
