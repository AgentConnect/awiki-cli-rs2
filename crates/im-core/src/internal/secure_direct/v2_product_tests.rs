...
fn index_entry(did: &str, mode: IdentityDeviceMode) -> crate::internal::identity_store::IndexEntry {
...
fn identity_summary(
    ...
    crate::identity::IdentitySummary {
        id: crate::ids::IdentityId::parse("recovered-identity").unwrap(),
...
#[test]
fn vnext_lookup_accepts_default_recovery_identity_without_local_alias() {
    let did = "did:example:recovered";
    ...
    index.credentials.insert(
        "alice".to_owned(),
        index_entry(did, IdentityDeviceMode::VNext),
    );

    let current = identity_summary(did, None, true);
    let entry = current_identity_index_entry(&index, &current).unwrap();
    assert_eq!(entry.did, did);
}

#[test]
fn vnext_lookup_accepts_unique_recovery_did_without_local_alias() {
    let did = "did:example:recovered";
    let mut index = crate::internal::identity_store::IndexPayload::default();
    index.credentials.insert(
        "alice".to_owned(),
    ...
    let current = identity_summary(did, None, false);
    assert!(current_identity_index_entry(&index, &current).is_some());
}

#[test]
fn vnext_lookup_rejects_ambiguous_did_without_local_alias() {
    let did = "did:example:shared";
    let mut index = crate::internal::identity_store::IndexPayload::default();
    index.credentials.insert(
        "alice".to_owned(),
        index_entry(did, IdentityDeviceMode::VNext),
    ...
    index.credentials.insert(
        "bob".to_owned(),
        index_entry(did, IdentityDeviceMode::Legacy),
    );

    let current = identity_summary(did, None, false);
    assert!(current_identity_index_entry(&index, &current).is_none());
}
