...
use crate::workspace_upgrade::legacy_sqlite::{self as store, rebind::rebind_local_identity_state};
...
fn rebind_local_identity_state_returns_zero_counts_when_database_missing() {
    ...
    let (store_rebind, e2ee_cleanup) = rebind_local_identity_state(&paths, "did:old", "did:new")
...
#[test]
fn rebind_local_identity_state_is_noop_and_preserves_rows() {
    ...
    let (store_rebind, e2ee_cleanup) =
        rebind_local_identity_state(&paths, " did:old ", " did:new ")
            .expect("legacy rebind is a compatibility no-op");

    assert!(store_rebind.values().all(|count| *count == 0));
    assert!(e2ee_cleanup.values().all(|count| *count == 0));
    ...
    assert_owner_count(&verify, "messages", "did:new", 0);
    ...
    let content: String = verify
        .query_row(
            ...
            rusqlite::params!["msg-1", "did:old"],
            |row| row.get(0),
        )
        .expect("noop keeps message content");
    assert_eq!(content, "hello");
}

#[test]
fn rebind_owner_did_noop_does_not_update_or_ignore_conflicts() {
    ...
    assert_owner_count(&connection, "relationship_events", "did:old", 1);
    ...
    assert_owner_count(&connection, "relationship_events", "did:new", 1);
}
...
#[test]
fn rebind_local_identity_state_existing_empty_database_is_noop() {
    ...
    let err = rebind_local_identity_state(&paths, "did:old", "did:new")
        .expect("existing DB without store tables is a compatibility no-op");

    assert!(err.0.values().all(|count| *count == 0));
    assert!(err.1.values().all(|count| *count == 0));
}
...
INSERT INTO messages (
    msg_id, owner_identity_id, owner_did, conversation_id, thread_id, direction, sender_did, receiver_did, content_type,
    content, stored_at, credential_name
) VALUES (?1, ?2, ?3, ?4, ?4, 0, ?5, ?3, 'text', ?6, ?7, 'default')
"#,
        rusqlite::params![
            "msg-1",
            "old-id",
...
INSERT INTO relationship_events (
    event_id, owner_identity_id, owner_did, target_did, event_type, status, created_at, updated_at, credential_name
...
"#,
    ...
    connection.execute(
        r#"
INSERT INTO groups (
    owner_identity_id, owner_did, group_id, name, group_mode, membership_status, stored_at, credential_name
) VALUES (?1, ?2, ?3, ?4, 'general', 'active', ?5, 'default')
"#,
        rusqlite::params![
            "old-id",
            "did:old",
...
INSERT INTO group_members (
    owner_identity_id, owner_did, group_id, user_id, member_did, status, last_synced_at, credential_name
) VALUES (?1, ?2, ?3, ?4, ?5, 'active', ?6, 'default')
"#,
        rusqlite::params![
            "old-id",
            "did:old",
...
"#,
    ...
    connection.execute(
        "INSERT INTO contacts (owner_identity_id, owner_did, did, handle) VALUES (?1, ?2, ?3, ?4)",
        rusqlite::params!["new-id", "did:new", "did:peer", "alice-new"],
    )?;
    connection.execute(
        r#"
INSERT INTO contact_handle_bindings (
