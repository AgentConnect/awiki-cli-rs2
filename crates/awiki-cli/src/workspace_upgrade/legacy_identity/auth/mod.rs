use crate::cli_http::{HttpClient, HttpRequest, HttpResponse, Profile};
use crate::cli_trace;
...
pub use wire::{
    build_json_rpc_payload, decode_json_rpc_response, http_status_error, CONTENT_TYPE_JSON,
};
...
impl Session {
    pub fn new(
        ...
        _identity_name: impl Into<String>,
        _did: impl Into<String>,
    ...
    pub fn ensure_jwt_profile_traced(
        ...
        self.remember_scope(request_url);
        let mut phase = cli_trace::ensure_jwt_phase(trace_operation);
    ...
    fn do_request_with_timeout(
        ...
        let mut phase = if trace_operation.trim().is_empty() {
            cli_trace::PhaseFinish::inactive()
        } else {
            cli_trace::rpc_phase(trace_operation)
        };
