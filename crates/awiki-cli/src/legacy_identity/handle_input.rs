...
pub fn complete_bare_handle(target: &str, did_domain: &str) -> String {
    ...
    let Ok(normalized) = normalize_handle_input(trimmed, did_domain) else {
        return trimmed.to_string();
    };
    if normalized.explicit_domain {
        trimmed.to_string()
    } else {
        normalized.full_handle
    }
}
