...
fn default_specs() -> &'static [CommandSpec] {
    &[
        ...
        CommandSpec { name: "id.register", use_: "register", short: "Register a handle-backed user identity", long: "Use --community for Open Server registration after capability verification. For commercial services, use --verification-stdin to keep phone verification material out of process arguments. The command reads one JSON object with phone and optional otp fields and never prints those fields.", aliases: &[], phase: "phase3", hidden: false, implemented: true, handler: "id.register", side_effect: true, outputs: &["json", "pretty"], flags: &[flag!("handle", "string", "Handle local part", required), flag!("community", "bool", "Register with an explicitly supported Community Home without contact verification"), flag!("phone", "string", "Phone number for registration"), flag!("email", "string", "Email address for registration"), flag!("otp", "string", "Verification code"), flag!("verification-stdin", "bool", "Read phone and optional OTP from one JSON object on stdin"), flag!("invite-code", "string", "Invite code if required"), flag!("wait", "bool", "Wait for email verification before completing registration")] },
        ...
        CommandSpec { name: "id.import-v1", use_: "import-v1", short: "Import credentials from the v1 awiki-agent-id-message layout", long: "", aliases: &[], phase: "phase2", hidden: false, implemented: true, handler: "id.import-v1", side_effect: true, outputs: &["json", "pretty"], flags: &[flag!("name", "string", "Import one legacy identity by name"), flag!("all", "bool", "Import all detected legacy identities"), flag!("credentials-dir", "string", "Explicit legacy credentials source for this import only")] },
