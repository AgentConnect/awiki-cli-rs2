...
fn default_specs() -> &'static [CommandSpec] {
    &[
        ...
        CommandSpec { name: "id.device.join.approve", use_: "approve", short: "Approve a verified Join and automatically configure management access", long: "Requires a foreground TTY. The user must type the locally derived SAS and APPROVE. The one-time approval handle is created and consumed in-process and never printed; the joined member is automatically provisioned by Core with at most three durable attempts. Delivery acceptance is separate from management readiness.", aliases: &[], phase: "phase3", hidden: false, implemented: true, handler: "id.device.join.approve", side_effect: true, outputs: &["json", "pretty"], flags: &[flag!("session", "string", "Join session id", required)] },
