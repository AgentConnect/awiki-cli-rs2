...
fn default_specs() -> &'static [CommandSpec] {
    &[
        ...
        CommandSpec { name: "id.register", use_: "register", short: "Register a handle-backed user identity", long: "Use --verification-stdin to keep phone verification material out of process arguments. The command reads one JSON object with phone and optional otp/invite_code fields and never prints those fields.", aliases: &[], phase: "phase3", hidden: false, implemented: true, handler: "id.register", side_effect: true, outputs: &["json", "pretty"], flags: &[flag!("handle", "string", "Handle local part", required), flag!("phone", "string", "Phone number for registration"), flag!("email", "string", "Email address for registration"), flag!("otp", "string", "Verification code"), flag!("verification-stdin", "bool", "Read phone and optional OTP/invite_code from one JSON object on stdin"), flag!("invite-code", "string", "Invite code if required"), flag!("wait", "bool", "Wait for email verification before completing registration")] },
