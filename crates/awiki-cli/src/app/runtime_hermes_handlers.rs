...
impl App {
    ...
    pub fn run_runtime_host_notify_hermes_setup(
        ...
        runtime::hermes_bridge::validate_local_notify_url(&notify_url).map_err(|err| {
            ExitError::new(
                "invalid_argument",
                2,
                err.to_string(),
            ...
            return self.render_success(
                ...
                json!({
                    "plan": {
                        ...
                        "deliver": deliver,
                        "previous_sink": runtime::resolve(&resolved).host_notify.sink,
...
fn resolve_hermes_deliver_target(resolved: &Resolved, override_value: &str) -> String {
    ...
    if !config_file.is_empty() {
        let (file_config, exists, error) = config::read_file_config(config_file);
...
fn resolve_hermes_notify_secret_for_setup(
    ...
    if !notify_url.trim().is_empty() {
        if let Ok(secret) = std::env::var(runtime::hermes_host_notify::HERMES_NOTIFY_SECRET_ENV) {
        ...
        if let Ok(secret) =
            std::env::var(runtime::hermes_host_notify::LEGACY_WEBHOOK_NOTIFY_SECRET_ENV)
        {
            let secret = secret.trim();
            if !secret.is_empty() {
                return Ok((secret.to_string(), "environment".to_string()));
...
fn resolve_hermes_home_dir() -> String {
    runtime::hermes_bridge::resolve_hermes_home().unwrap_or_else(|_| {
