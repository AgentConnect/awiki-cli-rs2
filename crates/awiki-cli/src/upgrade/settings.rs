...
pub fn parse_legacy_settings(raw: &[u8]) -> Result<NormalizedLegacySettings, LegacySettingsError> {
    ...
    .to_string();
    let user_service_url = config::normalize_base_url(legacy.user_service_url.trim());
    let molt_message_url = config::normalize_base_url(legacy.molt_message_url.trim());
