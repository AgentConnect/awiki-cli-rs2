...
use crate::cli_shell::legacy_sqlite::{self as store, StoreError};
use crate::diagnostics::legacy_sqlite as diagnostic_store;
...
impl App {
    ...
    pub fn run_debug_db_handle_history(&self, command: &ParsedCommand) -> Result<(), ExitError> {
        ...
        let rows = diagnostic_store::list_contact_handle_history(&db, &handle).map_err(|err| {
            store_exit(
                err,
