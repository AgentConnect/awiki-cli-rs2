...
impl App {
    pub fn run_runtime_status(&self) -> Result<(), ExitError> {
        ...
        let listener = host_runtime::current_listener_status(&resolved);
    ...
    pub fn run_runtime_mode_get(&self) -> Result<(), ExitError> {
        ...
        self.render_success(
            ...
            &resolved,
            json!({ "runtime": host_runtime::runtime_value(&resolved) }),
            "Runtime mode loaded",
            Vec::new(),
        )
    }
    ...
    fn run_runtime_host_notify_enable_toggle(
        ...
        workspace_config::update_host_notify_enabled(&resolved.paths, enabled)
            .map_err(internal_anyhow)?;
        let resolved = self.resolve_config()?;
        let host_notify =
    ...
    pub fn run_runtime_listener_uninstall(&self) -> Result<(), ExitError> {
        self.run_runtime_listener_lifecycle(
            ...
            host_runtime::uninstall_listener,
        )
    }
    ...
    pub fn run_runtime_host_notify_config_set(
        ...
        let normalized_sink = workspace_config::normalize_host_notify_sink_for_write(sink)
            .map_err(|_| {
                ExitError::new(
                    "invalid_argument",
                    2,
        ...
        workspace_config::update_host_notify_sink(&resolved.paths, &normalized_sink)
            .map_err(internal_anyhow)?;
        let resolved = self.resolve_config()?;
        let host_notify =
            host_runtime::host_notify_config_view(&resolved).map_err(internal_anyhow)?;
    ...
    pub fn run_runtime_host_notify_openclaw_set(
        ...
        host_runtime::validate_openclaw_hook_url(hook_url).map_err(|err| {
            ExitError::new(
                "invalid_argument",
                2,
                err.to_string(),
        ...
        workspace_config::update_openclaw_settings(&resolved.paths, Some(hook_url))
            .map_err(internal_anyhow)?;
        let resolved = self.resolve_config()?;
        let openclaw = host_runtime::host_notify_config_view(&resolved)
            .map_err(internal_anyhow)?
            .get("openclaw")
            .cloned()
            .unwrap_or_else(|| json!({}));
    ...
    pub fn run_runtime_host_notify_openclaw_set_token(
        ...
        workspace_config::set_openclaw_token(&resolved.paths, &token).map_err(internal_anyhow)?;
        let resolved = self.resolve_config()?;
        let (listener, warnings) =
            refresh_listener_for_host_notify_change(&resolved).map_err(internal_anyhow)?;
        self.render_success(
    ...
    pub fn run_runtime_host_notify_openclaw_clear_token(&self) -> Result<(), ExitError> {
        ...
        workspace_config::clear_openclaw_token(&resolved.paths).map_err(internal_anyhow)?;
        let resolved = self.resolve_config()?;
        let (listener, warnings) =
            refresh_listener_for_host_notify_change(&resolved).map_err(internal_anyhow)?;
        self.render_success(
    ...
    pub fn run_runtime_host_notify_openclaw_route_add(
        ...
        let route_registry_path = host_runtime::openclaw_routes::routes_path(&resolved.paths);
        if self.globals.dry_run {
            return self.render_success(
        ...
        let (route, added, routes) =
            host_runtime::openclaw_routes::add_route(&resolved.paths, route)
                .map_err(internal_anyhow)?;
        ...
        if added {
            match host_runtime::openclaw_webhook::send_route_confirmation(&resolved, &route) {
                Ok(confirmation) => {
    ...
    pub fn run_runtime_host_notify_openclaw_route_list(&self) -> Result<(), ExitError> {
        let resolved = self.resolve_config()?;
        let routes =
            host_runtime::openclaw_routes::load_routes(&resolved.paths).map_err(internal_anyhow)?;
        self.render_success(
    ...
    pub fn run_runtime_host_notify_openclaw_route_remove(
        ...
        let (route, removed, routes) =
            host_runtime::openclaw_routes::remove_route(&resolved.paths, route)
                .map_err(internal_anyhow)?;
...
fn resolve_openclaw_route_from_flags(
    command: &ParsedCommand,
) -> Result<host_runtime::openclaw_routes::Route, ExitError> {
    host_runtime::openclaw_routes::resolve_route_input(
        command
            .flags
            .get("channel")
            .map(String::as_str)
