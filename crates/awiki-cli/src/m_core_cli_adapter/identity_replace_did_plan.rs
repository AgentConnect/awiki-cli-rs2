...
const REPLACE_DID_DANGER_WARNING: &str =
    "Dangerous command: replacing a DID rewrites local identity ownership references. Review the plan and backup path before executing.";
...
fn replace_did_plan_command_result(plan: ReplaceDidPlan) -> Result<CommandResult, ExitError> {
