...
pub async fn get_root_async(
    ...
    let domain = SiteDomain::parse(domain)?;
    let root = client.site().get_root_async(domain.clone()).await?;
    Ok(CommandResult {
        data: json!({
            "action": "site_root_get",
            "identity": identity_value(client.current_identity()),
...
pub async fn get_page_async(
    ...
    let page_ref = page_ref(domain, slug)?;
    let page = client.site().get_page_async(page_ref.clone()).await?;
    Ok(page_action_result(
        "site_page_get",
        client.current_identity(),
        page.raw,
...
pub async fn update_page_async(
    ...
    let page = client
        .site()
        .update_page_async(page_ref.clone(), SitePageUpdate { body })
        .await?;
    Ok(page_action_result(
        "site_page_update",
        client.current_identity(),
...
pub async fn rename_page_async(
    ...
    let page = client
        .site()
        .rename_page_async(page_ref.clone(), target.clone())
        .await?;
    Ok(CommandResult {
        data: json!({
            "action": "site_page_rename",
...
pub async fn delete_page_async(
    ...
    let page_ref = page_ref(domain, slug)?;
    let result = client.site().delete_page_async(page_ref.clone()).await?;
    Ok(CommandResult {
        data: json!({
            "action": "site_page_delete",
            "identity": identity_value(client.current_identity()),
