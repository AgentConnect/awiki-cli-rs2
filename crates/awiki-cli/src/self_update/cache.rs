...
fn read_cache(
    ...
    if !file.policy_origin.trim().is_empty()
    ...
    Ok(Some(CacheRead {
        metadata: Metadata {
            product: if file.product.trim().is_empty() {
                "awiki-cli".to_string()
            } else {
                file.product.trim().to_string()
            },
            channel: if file.channel.trim().is_empty() {
                "stable".to_string()
            } else {
                file.channel.trim().to_string()
            },
            policy_origin: if file.policy_origin.trim().is_empty() {
                expected_policy_origin.to_string()
            } else {
                file.policy_origin.trim().to_string()
            },
            policy_revision: file.policy_revision.max(1),
