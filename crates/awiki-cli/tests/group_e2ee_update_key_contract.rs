...
#[test]
fn group_e2ee_update_key_rpc_deterministic_submit_failure_aborts_pending_update_like_go() {
    let workspace = TempDir::new("group-e2ee-update-submit-rpc-2001").expect("workspace");
    register_ready_group_identity(workspace.path(), IDENTITY, "alice", "jwt-alice");
    let bin_dir = TempDir::new("group-e2ee-update-submit-rpc-2001-bin").expect("bin dir");
    let fake_mls = bin_dir.path().join("anp-mls");
    let args_log = workspace.path().join("mls-args.log");
    let stdin_log = workspace.path().join("mls-stdin.jsonl");
    write_fake_anp_mls_group_update_key_with_terminal(
    ...
    let server = TestServer::new(vec![
        ...
        TestResponse::ok(&json_rpc_result(update_key_package())),
        TestResponse::ok(&json_rpc_error(2001, "deterministic update rejection")),
    ]);
    write_group_config(workspace.path(), &server.base_url());
    ...
    assert_text_contains(
        error["error"]["message"].as_str().expect("error message"),
        "service rpc error 2001: deterministic update rejection",
    );
    assert_text_contains(
        error["error"]["message"].as_str().expect("error message"),
        "local group E2EE update-key pending commit aborted",
...
#[test]
fn group_e2ee_update_key_retryable_submit_failure_retains_pending_update_like_go() {
    let workspace = TempDir::new("group-e2ee-update-submit-500").expect("workspace");
    register_ready_group_identity(workspace.path(), IDENTITY, "alice", "jwt-alice");
    let bin_dir = TempDir::new("group-e2ee-update-submit-500-bin").expect("bin dir");
    let fake_mls = bin_dir.path().join("anp-mls");
    let args_log = workspace.path().join("mls-args.log");
    let stdin_log = workspace.path().join("mls-stdin.jsonl");
    write_fake_anp_mls_group_update_key_with_terminal(
    ...
    assert_eq!(
        provider_commands(&args_log),
        vec!["group update-member-prepare"]
    );
    let provider_stdin = provider_stdin_jsonl(&stdin_log);
