...
const IDENTITY: &str = "alice-group-e2ee-recover";
const AGENT_DID: &str = "did:wba:awiki.ai:alice:e1_alice";
const GROUP_DID: &str = "did:wba:awiki.ai:groups:recover:e1_group";
const MEMBER_DID: &str = "did:wba:awiki.ai:bob:e1_bob";
const SERVICE_DID: &str = "did:wba:awiki.ai:service:e1_message";

#[test]
fn group_e2ee_recover_member_live_leases_recovery_key_package_prepares_hidden_recover_and_finalizes_without_p4_add(
) {
    let workspace = TempDir::new("group-e2ee-recover-live").expect("workspace");
    register_ready_group_identity(workspace.path(), IDENTITY, "alice", "jwt-alice");
    let bin_dir = TempDir::new("group-e2ee-recover-bin").expect("bin dir");
    ...
    write_fake_anp_mls_group_recover_member(&fake_mls, &args_log, &stdin_log);
    ...
    assert_eq!(provider_stdin[0]["params"]["p4_membership_mutate"], false);
    assert_eq!(
        provider_stdin[0]["params"]["recovery_operation_purpose"],
        "same-device-crypto-recovery"
    );
    assert!(
    ...
    assert_eq!(
        envelope["summary"],
        "Recovered active group E2EE member without P4 membership mutation"
    );
    ...
    assert_eq!(
        envelope["data"]["recovery_key_package"]["key_package_id"],
        "kp-bob-recovery"
    );
    assert_eq!(
        envelope["data"]["recovery_key_package"]["private_material"],
        false
    );
    assert!(
        envelope["data"]["recovery_key_package"]
            .get("private_key_package_b64u")
            .is_none(),
    ...
    assert!(
        ...
        "recover-member must not call public P4 group.add"
    );
    ...
    assert_eq!(bodies[1]["params"]["body"]["purpose"], "recovery");
    ...
    assert_eq!(
        bodies[2]["params"]["body"]["recovery_key_package_id"],
        "kp-bob-recovery"
    );
    assert_eq!(
        bodies[2]["params"]["body"]["group_key_package"]["purpose"],
        "recovery"
    );
    assert_eq!(
        bodies[2]["params"]["body"]["group_state_ref"]["group_did"],
        GROUP_DID
    );
    assert_eq!(
    ...
    assert_eq!(
        bodies[2]["params"]["body"]["commit_b64u"],
        "cmVjb3Zlci1jb21taXQ"
    );
    assert_eq!(
        bodies[2]["params"]["body"]["welcome_b64u"],
        "cmVjb3Zlci13ZWxjb21l"
    );
    assert!(
        bodies[2]["params"]["body"].get("member_did").is_none(),
        "recover_member body must not carry P4 member_did"
    );
    assert!(
        bodies[2]["params"]["body"].get("role").is_none(),
        "recover_member body must not carry P4 role"
    );
    assert!(
        bodies[2]["params"]["body"]["group_key_package"]
            .get("private_key_package_b64u")
            .is_none(),
        "hidden recover RPC must not leak private KeyPackage material"
    );
}
...
fn recovery_key_package() -> Value {
    json!({
        "leased": true,
        "lease_id": "lease-bob-recovery",
        "group_key_package": service_leased_recovery_key_package(),
        "key_package": service_leased_recovery_key_package(),
        "key_package_id": "kp-bob-recovery",
        "expires_at": "2030-01-01T00:00:00Z"
    })
}

fn service_leased_recovery_key_package() -> Value {
    json!({
        "owner_did": MEMBER_DID,
        "device_id": "bob-main",
        "key_package_id": "kp-bob-recovery",
        "suite": "MLS_128_DHKEMX25519_AES128GCM_SHA256_Ed25519",
        "mls_key_package_b64u": "bWxzLXJlY292ZXJ5LWtleS1wYWNrYWdl",
        "did_wba_binding": {
            ...
            "proof": {
                ...
                "proofValue": "zproof-bob-recovery"
            }
        },
        "expires_at": "2030-01-01T00:00:00Z",
        "purpose": "recovery"
...
fn write_fake_anp_mls_group_recover_member_with_terminal(
    ...
    let recover_response = json!({
        ...
        "result": {
            "operation_id": "op-mls-recover-prepare",
            "pending_commit_id": "pc-recover-1",
            "recovery_key_package_id": "kp-bob-recovery",
            "crypto_group_id_b64u": "Y3J5cHRv",
            "from_epoch": "5",
            "to_epoch": "6",
            "epoch": "6",
            "epoch_authenticator_b64u": "YXV0aDY",
            "commit_b64u": "cmVjb3Zlci1jb21taXQ",
            "welcome_b64u": "cmVjb3Zlci13ZWxjb21l",
    ...
    let finalize_ok_response = json!({
        ...
        "result": {
            "crypto_group_id_b64u": "Y3J5cHRv",
            "epoch": "6",
            "finalized_commit_id": "fc-recover-1",
            "epoch_authenticator_b64u": "YXV0aDY",
            "group_state_ref": {
                "group_did": GROUP_DID,
                "group_state_version": "v6",
...
body=$(cat)
printf '%s\n' "$body" >> {stdin_log}
if [ "$1" = "group" ] && [ "$2" = "recover-member-prepare" ]; then
  printf '%s\n' {recover_response}
  exit 0
fi
