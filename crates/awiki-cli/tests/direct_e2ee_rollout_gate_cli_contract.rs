...
#[test]
fn direct_gate_defaults_off_and_accepts_explicit_zero_or_one() {
    for gate in [None, Some("0"), Some("1")] {
        let workspace = TempDir::new("direct-rollout-valid").unwrap();
...
#[test]
fn invalid_direct_gate_fails_closed() {
    for gate in ["", "true", "yes", "2"] {
        let workspace = TempDir::new("direct-rollout-invalid").unwrap();
        write_file_compat_config(workspace.path());
        ...
        assert!(error["error"]["message"]
            .as_str()
            .unwrap()
            .contains("AWIKI_MULTI_DEVICE_DIRECT_E2EE_ENABLED must be 0 or 1"));
    }
}
...
fn awiki_cmd(workspace: &Path, gate: Option<&str>) -> Output {
    ...
    if let Some(gate) = gate {
        command.env("AWIKI_MULTI_DEVICE_DIRECT_E2EE_ENABLED", gate);
    }
