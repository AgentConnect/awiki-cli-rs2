...
use awiki_cli::runtime::listener_bridge_runtime::{
    ...
    host_notify_for_bridge_created_session, local_notifications_for_bridge_created_session,
    wait_for_bridge_session_bootstrap_with_timeout, BridgeSessionBootstrapResult,
};
use awiki_cli::runtime::listener_local_notifications::LocalNotificationQueue;
...
#[test]
fn bridge_created_sessions_reuse_supervisor_local_notification_queue_like_go() {
    let supervisor_queue = Arc::new(Mutex::new(LocalNotificationQueue::default()));

    let session_queue = local_notifications_for_bridge_created_session(&supervisor_queue);

    assert!(Arc::ptr_eq(&session_queue, &supervisor_queue));
    assert_eq!(Arc::strong_count(&supervisor_queue), 2);
}
