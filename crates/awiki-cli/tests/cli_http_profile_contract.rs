use awiki_cli::cli_http::{self, Profile};
use std::sync::Mutex;
...
fn transport_timeout_for_profile_name_falls_back_to_rpc_default() {
    ...
    let config = cli_http::resolve();
    assert_eq!(
        cli_http::timeout_for_profile_name(&config, "rpc_default"),
        Duration::from_secs(31)
    );
    assert_eq!(
        cli_http::timeout_for_profile_name(&config, "unknown"),
        Duration::from_secs(31)
    );
    assert_eq!(
        cli_http::timeout_for_profile_name(&config, "auth_refresh"),
        Duration::from_secs(20),
