...
fn http_client_ca_bundle_errors_match_go_messages() {
    ...
    let err = transportcfg::new_http_client(&path_string(&missing)).expect_err("missing bundle");
    assert!(
    ...
    let err = transportcfg::new_http_client(&path_string(&invalid)).expect_err("invalid bundle");
    assert_eq!(
        err.to_string(),
...
fn http_client_appends_valid_ca_bundle_to_webpki_roots() {
    ...
    let client = transportcfg::new_http_client(&path_string(&bundle)).expect("client");
...
fn http_client_default_ignores_proxy_env_like_go_transportcfg_client() {
    ...
    let response = transportcfg::new_http_client("")
        .expect("client")
...
fn http_client_proxy_env_uses_absolute_form_for_http_proxy() {
    ...
    let response = transportcfg::new_http_client_with_proxy_env("")
        .expect("client")
