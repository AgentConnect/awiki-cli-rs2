...
#[test]
fn mail_live_command_is_blocked_before_legacy_mail_rpc() {
    ...
    assert_cutover_unsupported(&output, "mail.inbox", "mail");
    assert!(
        server.requests().is_empty(),
        "mail cutover guard must not reach legacy mail RPC server"
    );
}

#[test]
fn mail_attachment_live_command_is_blocked_before_writing_output() {
    let workspace = TempDir::new().expect("workspace");
    register_ready_mail_identity(workspace.path(), "alice-mail", "alice", "jwt-mail");
    let server = TestServer::new(vec![TestResponse::ok(
        r#"{"jsonrpc":"2.0","result":{"filename":"out.txt","content_base64":"aGVsbG8="},"id":"req-1"}"#,
    )]);
    write_mail_config(workspace.path(), &server.base_url());
    let output_path = workspace.path().join("out.txt");
    ...
    assert_cutover_unsupported(&output, "mail.attachment.download", "mail");
    assert!(
        !output_path.exists(),
        "unsupported mail attachment command must not write output files"
    );
    assert!(
        server.requests().is_empty(),
        "mail attachment cutover guard must not reach legacy mail RPC server"
    );
}
...
impl TempDir {
    fn new() -> std::io::Result<Self> {
        ...
        let path = std::env::temp_dir().join(format!(
            "awiki-cli-rs2-mail-live-cutover-test-{}-{nanos}",
            std::process::id()
        ));
