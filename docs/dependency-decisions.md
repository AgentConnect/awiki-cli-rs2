...
| Workspace UpgradeIfNeeded no-op/current preflight slice | Add no dependency for the first Go `internal/upgrade/upgrader.go` `UpgradeIfNeeded` orchestration boundary; reuse existing inspect/meta/journal helpers and the existing upgrader planner. | This slice translates only the non-mutating/current-version branch: context-required error, inspect/current-meta capture, newer-than-supported rejection, empty/latest no-op return, and stale journal clearing. It explicitly defers lock acquisition, backup creation/reuse, journal phase loop, migration apply/validate, meta stamping after migrations, identity replacement RPC, legacy SQLite import, rollback, and cleanup so migration execution remains separately testable. | `cargo +1.79.0 test -p awiki-cli upgrade::upgrader --locked`; `cargo +1.79.0 test -p awiki-cli --test workspace_upgrade_contract workspace_upgrade_if_needed --locked`; full verification recorded in `docs/verification/`. No dependency was added. |
