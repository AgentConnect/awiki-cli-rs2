...
| Workspace UpgradeIfNeeded journal phase loop slice | Add no dependency for the next Go `internal/upgrade/upgrader.go` `UpgradeIfNeeded` orchestration boundary; reuse existing inspect/meta/journal/backup/lock helpers and the existing migration trait implementations. | This slice translates the migration loop after backup setup: journal `checking`/`applying`/`validating` saves, migration `is_done`/`apply`/`validate` calls, meta stamping after each successful migration, `Context.current_meta` refresh, and final journal clear after full success. It still defers v0->v1 default apply, v2->v3 k1->e1 DID replacement, rollback, and service-backed identity replacement so those dependency-sensitive behaviors remain separately testable. | `cargo +1.79.0 test -p awiki-cli --test workspace_upgrade_if_needed_contract --locked`; `cargo +1.79.0 test -p awiki-cli --test workspace_upgrade_contract --locked`; full verification recorded in `docs/verification/`. No dependency was added; it reuses existing Rustls-free local upgrade helpers, the documented bundled SQLite backup path, and direct lock FFI, with no new OpenSSL/native-tls/platform service path. |
