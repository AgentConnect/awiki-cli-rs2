...
| Listener notification side-effect executor slice | Add no new dependency for Go `internal/runtime/listener/server.go` `handleNotification` local side-effect execution; reuse existing `rusqlite + bundled`, store DAO helpers, listener contact sync, route planner, `HostNotifySink`, and listener status helpers. | This slice intentionally crosses from pure planning into local SQLite/contact/host-sink execution, but all required dependencies are already approved in the port: SQLite uses the accepted bundled `rusqlite` path and host notification sinks use existing std/Rustls-backed implementations. The dispatch outcome/status-update signal is local enum/control-flow state and avoids adding a foreground `Supervisor`, WebSocket transport, async runtime, service-manager library, Windows named-pipe crate, new HTTP/TLS client, OpenSSL/`native-tls`, bundled OpenSSL, YAML crate, or new E2EE provider dependency. Later foreground listener wiring must still reuse this executor behind a Rustls-first WebSocket transport decision. | `cargo +1.79.0 test -p awiki-cli --test runtime_listener_notification_execute_contract --locked`; adjacent planner/contact/store/host-notify/status checks; Cargo manifests and lockfile unchanged. No dependency was added. |
