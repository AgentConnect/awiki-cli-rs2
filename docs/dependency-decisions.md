...
| Workspace UpgradeIfNeeded lock/pre-migration slice | Add no dependency for the next Go `internal/upgrade/upgrader.go` `UpgradeIfNeeded` orchestration boundary; reuse existing inspect/meta/journal helpers, the existing upgrader planner, and the already translated direct file-lock helper. | This slice translates the pre-migration branch through lock acquisition and lock-inside reinspection: context-required error, inspect/current-meta capture, newer-than-supported rejection, empty/latest no-op return, stale journal clearing, direct file-lock acquisition before real migration execution, lock-held rejection, lock release on deferred return, and second inspect/current-meta refresh while locked. It still explicitly defers backup creation/reuse, journal phase loop, migration apply/validate, meta stamping after migrations, identity replacement RPC, legacy SQLite import, rollback, and cleanup through the migration loop so execution remains separately testable. | `cargo +1.79.0 test -p awiki-cli --test workspace_upgrade_contract workspace_upgrade_if_needed --locked`; full verification recorded in `docs/verification/`. No dependency was added; direct lock FFI was already documented in the file-lock slice, and no new OpenSSL/native-tls/platform service path was introduced. |
...
| Workspace v0->v1 local apply composition slice | Add no dependency for composing already translated local v0->v1 steps; reuse config writer, identity import, bundled SQLite import/schema helpers, and resolved-config refresh. | This slice proves Go ordering across local operations without selecting HTTP/TLS/authsdk dependencies or enabling remote k1->e1 DID replacement. Full `Migration::apply`/`UpgradeIfNeeded` execution remains separate because it still needs journal, backup, meta stamping, rollback, cleanup, and remote DID replacement evidence. | Focused v0->v1 migration contract tests, related identity/store/upgrade tests, full `cargo +1.79.0 test -p awiki-cli --locked`, Go focused upgrade tests, structure check, build, and dependency audit passed. No new dependency was added; audit remains limited to existing Rustls/update paths and the approved `rusqlite + bundled` SQLite path. |
