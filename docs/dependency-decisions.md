...
| Listener notification side-effect executor slice | Add no new dependency for Go `internal/runtime/listener/server.go` `handleNotification` local side-effect execution; reuse existing `rusqlite + bundled`, store DAO helpers, listener contact sync, route planner, and `HostNotifySink`. | This slice intentionally crosses from pure planning into local SQLite/contact/host-sink execution, but all required dependencies are already approved in the port: SQLite uses the accepted bundled `rusqlite` path and host notification sinks use existing std/Rustls-backed implementations. Adding WebSocket crates, async runtime crates, platform service-manager libraries, Windows named-pipe crates, new HTTP/TLS clients, OpenSSL/`native-tls`, bundled OpenSSL, YAML crates, or new E2EE provider dependencies would mix foreground session transport, service control, or secure decrypt execution into a single-notification side-effect boundary. Later foreground listener wiring must still reuse this executor behind a Rustls-first WebSocket transport decision. | `cargo +1.79.0 test -p awiki-cli --test runtime_listener_notification_execute_contract --locked`; adjacent planner/contact/store/host-notify checks; Cargo manifests and lockfile unchanged. No dependency was added. |
