...
| Workspace UpgradeIfNeeded backup setup slice | Add no dependency for the next Go `internal/upgrade/upgrader.go` `UpgradeIfNeeded` orchestration boundary; reuse existing inspect/meta/journal helpers, the existing upgrader planner, the already translated direct file-lock helper, and the existing backup helper. | This slice translates the pre-migration branch through lock acquisition, lock-inside reinspection, journal reload, journal `backup_dir` reuse, new backup creation when no journal backup exists, and `Context.backup_dir` capture. It still explicitly defers journal phase loop, migration `is_done`/apply/validate, meta stamping after migrations, identity replacement RPC, legacy SQLite import, rollback, and cleanup through the migration loop so execution remains separately testable. | `cargo +1.79.0 test -p awiki-cli --test workspace_upgrade_contract workspace_upgrade_if_needed --locked`; full verification recorded in `docs/verification/`. No dependency was added; it reuses the already documented bundled SQLite backup path and direct lock FFI, with no new OpenSSL/native-tls/platform service path. |
