...
| Listener notification side-effect executor slice | Add no new dependency for Go `internal/runtime/listener/server.go` `handleNotification` local side-effect execution; reuse existing `rusqlite + bundled`, store DAO helpers, listener contact sync, route planner, `HostNotifySink`, and listener status helpers. | This slice intentionally crosses from pure planning into local SQLite/contact/host-sink execution, but all required dependencies are already approved in the port: SQLite uses the accepted bundled `rusqlite` path and host notification sinks use existing std/Rustls-backed implementations. The dispatch outcome/status-application path is local enum/control-flow state plus the existing status writer and avoids adding a foreground `Supervisor`, WebSocket transport, async runtime, service-manager library, Windows named-pipe crate, new HTTP/TLS client, OpenSSL/`native-tls`, bundled OpenSSL, YAML crate, or new E2EE provider dependency. Later foreground listener wiring must still reuse this executor behind a Rustls-first WebSocket transport decision. | `cargo +1.79.0 test -p awiki-cli --test runtime_listener_notification_execute_contract --locked`; adjacent planner/contact/store/host-notify/status checks; Cargo manifests and lockfile unchanged. No dependency was added. |
| Listener notification local handler slice | Add no new dependency for the local composition subset of Go `internal/runtime/listener/server.go` `Supervisor.handleNotification`; reuse `rusqlite + bundled`, the executor, `HostNotifySink`, `NoopHostNotifySink`, and listener status helpers. | This slice models the future foreground handler boundary without constructing `Supervisor`, locks, sessions, WebSocket clients, signal loops, service managers, Windows named pipes, a new HTTP/TLS stack, OpenSSL/`native-tls`, bundled OpenSSL, YAML crates, async runtime crates, or a new E2EE provider. The only new behavior beyond the executor is local control flow for Go's `s.hostNotify == nil` early return: local persistence/contact side effects still run through existing SQLite helpers, while host-notify status mutation remains unchanged. Later real foreground wiring must still pass a real Rustls-first WebSocket notification stream and status mutex boundary into this helper. | `cargo +1.79.0 test -p awiki-cli --test runtime_listener_notification_handler_contract --locked`; adjacent executor/status checks; Cargo manifests and lockfile unchanged. No dependency was added. |
| Listener host-notify status mutation slice | Add no dependency for Go `internal/runtime/listener/server.go` `setHostNotifyError` and `clearHostNotifyError`; reuse the existing listener status JSON writer. | Host-notify delivery `LastError` mutation is local status-file behavior: set only writes when the string changes, clear only writes when non-empty, status-file write errors are ignored like Go's `_ = writeStatus(...)`, and the full status snapshot is persisted to `listener.status.json` on the successful write path. Adding WebSocket crates, async runtime crates, platform service-manager libraries, Windows named-pipe crates, new HTTP/TLS clients, OpenSSL/`native-tls`, bundled OpenSSL, YAML crates, or new SQLite dependencies would mix foreground listener execution into a small status helper boundary. | Focused listener service contract test plus adjacent listener status/notification executor checks; Cargo manifests and lockfile unchanged. No dependency was added. |
...
| Listener identity discovery helper slice | Add no dependency for the pure control-flow subset of Go `internal/runtime/listener/server.go` `startKnownSessions` and `watchNewIdentities`; use `std::time::Duration` and `std::collections::BTreeSet` only. | Identity discovery can be represented as event-to-action planning before constructing a real `Supervisor`: manager-list errors, context cancellation, existing-session skips, ensure-session attempts, original ensure-error text recording, immediate refresh via `recordSessionError`, duplicate same-tick identity handling, final refresh-status timing, and the 3-second watch interval are deterministic once identity summaries and injected ensure outcomes are supplied. Adding WebSocket crates, async runtime crates, platform service libraries, real identity filesystem access, SQLite access, HTTP/TLS clients, OpenSSL/`native-tls`, bundled OpenSSL, YAML crates, or E2EE provider dependencies would mix later session bootstrap/runtime execution into this helper-only slice. | `cargo +1.79.0 test -p awiki-cli --test runtime_listener_identity_watch_contract --locked`; adjacent listener session-state/session-loop checks; Cargo manifests and lockfile unchanged. No dependency was added. |
