...
| Config writer helper slice | Add no dependency for `internal/config/write.go` helper mutators; extend the existing hand-written config parser/writer. | This bounded slice translates Go writer helper field mutation behavior, schema-version stamping, Hermes persistence, and legacy webhook double-writes without changing the YAML dependency surface. Full YAML parser/serializer parity and atomic durable write semantics need separate dependency/platform decisions. | `crates/awiki-cli/tests/config_writer_contract.rs`, `core_contract`, and `runtime_contract` passed. Dependency tree unchanged except the approved bundled SQLite path; no OpenSSL/native-tls or new platform dependency was added. |
