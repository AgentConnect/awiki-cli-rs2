...
P5 发起端等待首次会话确认时，`pn=0,n>0` 的后续密文可能先被并发处理。ANP 完成结构、
设备、session 和 suite 校验后若因缺少 `n=0` 首次回复返回 `BadInitMessage`，Core 将其
保留为 `p5.session_pending`，沿已有退避调度等待确认提交后重新解密。等待不推进 ratchet、
replay 或消息投影；真正的 `n=0` 验证失败和错误安全绑定仍然拒绝。普通和 secret JSON
接收遵循同一规则，未认证的后续密文仍受接收表容量和保留期限制。

P5 的合法接收与其他收发可能同时读取同一 ratchet revision。提交 CAS 失败时，只有相同
owner/DID/local device/peer device/session 的活动记录确实已推进到较新 revision，才返回
`p5.session_revision_changed` 并沿已有输入退避重试；重试重新加载已提交 ratchet，再执行
密文验证和原子提交。竞争失败不提交 replay、消息或输入完成标记，也不回退接收游标。
不存在、已禁用、绑定变化或请求了未来 revision 的会话，以及密文/重放验证失败，仍保持安全拒绝。
