...
| Group service RPC boundary | `internal/cli/group.go`, `internal/message/group_service.go` | Non-dry-run group commands require registered identity auth, message service RPC, group membership state, and later group E2EE surfaces. | Current Rust slice translates `group create/update --dry-run` request-shape contracts only. Remote group lifecycle is deferred to the shared authsdk/message service slice. | deferred_translation |
