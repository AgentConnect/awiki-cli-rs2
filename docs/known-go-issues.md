...
| Group E2EE execution boundary | `internal/cli/group_e2ee.go`, `internal/message/group_e2ee_service.go` | Non-dry-run group E2EE commands require MLS provider state, hidden P6 service APIs, focused system-test gating, and stricter storage/security checks. | Current Rust slice translates static `group e2ee ... --dry-run` plans and schema only. Real `anp-mls` execution, hidden service RPCs, and storage/security validation remain a separate MLS/provider slice. | deferred_translation |
