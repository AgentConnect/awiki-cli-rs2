# SDK Refactor 2：实施版 IM SDK 与 CLI 边界方案
...
`sdk-refactor-2` 是对 `docs/sdk-refactor/` 的实施版补充：它不替代主方案，而是把主方案中的第一阶段拆成更容易执行的 SDK public API、CLI adapter 和迁移步骤。

`awiki-cli-rs2` 后续仍拆成两个 crate：
...
`im-core` 不是 wire helper、RPC params builder、SQLite helper、crypto helper 的集合。它应该是 **IM 产品能力 SDK**：调用方选择一个身份，随后调用身份鉴权、Handle 注册、私聊消息、群聊消息等高层能力。CLI 与 App 都只做适配，不直接拼 actor、auth path、owner DID、RPC payload、WebSocket frame 或 SQLite 查询。
...
```text
Phase 1 MVP：core 框架 + 多身份 + 身份鉴权 + Handle 注册 + 私聊文本 + 群聊文本 + 必要 inbox/history
```
...
这些能力保留在后续阶段迁移。这样可以先验证 `crates/im-core`、`ImCore` / `ImClient`、显式路径参数、多身份、auth、消息发送这条主链路，而不是一开始就被加密、附件、runtime daemon、group lifecycle 和 provider 抽象拖大。

## 3. 阅读顺序

1. [整体架构](architecture.md)：crate 边界、Phase 1 MVP、多身份模型、路径参数、blocking-first、public/internal 边界。
2. [公共接口](public-api.md)：按阶段标注的 `ImCore`、`ImClient`、identity/auth/messages/groups/local_state 接口草案。
