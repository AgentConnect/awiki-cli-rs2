...
`id register` 在调用 Core 前通过 User Service `registration_check` 先判断账号状态。已有短账号继续进入 Join/Recovery；只有新账号按服务端规则要求邀请码。预检查失败不会创建本地 pending 注册状态。服务端必须提供该接口，无法确认策略时不会继续发送验证码或注册。
手机号注册可以使用 `--verification-stdin` 从标准输入读取 JSON，字段为 `phone`、可选 `otp`、可选 `invite_code`。获取验证码和完成注册两次调用都应保留相同邀请码；不要把真实验证码或邀请码写进命令历史。标准输入模式不能混用这些字段的命令行参数。最终注册与邀请码消费仍由服务端原子校验。
