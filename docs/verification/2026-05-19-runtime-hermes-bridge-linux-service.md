...
explicitly gated for Rust Linux user-systemd service management, installs and
starts a real service-managed Hermes bridge and that follow-up status reports
the bridge as installed and running.

This is a Linux user-systemd system-test evidence batch. It does not claim
macOS launchd, Windows Service Manager, `bridge_available=true` health
readiness, real Hermes final delivery, or mail acceptance.
...
Observed result: 1 passed in 15.67s, 0 failed, 0 skipped.
...
- Uses local notify URL `http://127.0.0.1:8644/notify`, deliver target
  `telegram`, and an explicit secret.
...
- Verifies the bridge status in setup reports `installed=true`,
  `running=true`, `service_platform=linux-systemd`, and a deterministic
  `awiki-cli-hermes-bridge-<12 hex>` service name.
- Runs `runtime host-notify hermes status` through the same gated service
  environment and verifies installed/running/Linux-systemd status again.
...
- This does not assert `bridge_available=true`. A prior live probe showed the
  service can be active while the health endpoint returns not ready, so health
  readiness remains a separate Hermes adapter/service investigation.
