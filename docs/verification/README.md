...
Scope: wire Go `internal/cli/id.go` `identityService()` workspace-upgrade
behavior into Rust `id bind`. Go constructs `identityService()` before dry-run
planning or `Service.Bind`, and `Service.Bind` requires an active identity
before phone/email exactly-one validation. Rust therefore must migrate legacy
`config.json` before the local active-identity lookup boundary.

Rust change:

- `crates/awiki-cli/src/app.rs`: `id bind` now calls
  `resolve_config_for_workspace()` before dry-run planning or live bind.
- `crates/awiki-cli/tests/identity_contract.rs`: added
  `identity_bind_migrates_legacy_config_json_before_active_identity_boundary_like_go`
  using the shared empty-identity-boundary workspace-upgrade helper.

System-test change:

- Added
  `tests_v2/id/test_identity_cli.py::test_id_bind_rust_migrates_legacy_config_json_before_identity_boundary`.
- Refactored the repeated identity legacy-config migration assertions in
  `tests_v2/id/test_identity_cli.py` into local helper functions, reducing the
  file from 1179 lines to 1125 while preserving existing profile-get,
  profile-set, and refresh-token selectors.
- Updated `tests_v2/id/CLAUDE.md` to mention the Rust `id bind` legacy config
  workspace-upgrade coverage.
...
cd /home/ecs-user/awiki-space/awiki-cli-rs2
cargo +1.79.0 fmt --check
cargo +1.79.0 test -p awiki-cli --test identity_contract identity_bind_migrates_legacy_config_json_before_active_identity_boundary_like_go --locked
cargo +1.79.0 test -p awiki-cli --test identity_contract --locked
cargo +1.79.0 check -p awiki-cli --locked
cargo +1.79.0 run --bin xtask --locked -- check-structure
cargo +1.79.0 tree --workspace --locked | rg -i 'openssl|native-tls|openssl-sys|openssl-probe|openssl-src|reqwest|hyper|rustls|webpki|aws-lc|ring|libsqlite3-sys|sqlite|pkg-config|vcpkg|cc |systemd|dbus|launchd|kardianos|service-manager|tungstenite|websocket|serde_yaml|yaml|hmac|sha2|base64'
...
```text
cd /home/ecs-user/awiki-space/awiki-cli
go test ./internal/cli ./internal/identity ./internal/upgrade -run 'Test.*Bind|Test.*Identity|Test.*Config|TestUpgradeIfNeededMigratesLegacyConfigJSON|TestUpgradeIfNeededStampsCurrentWorkspaceMetadata' -count=1
```

```text
cd /home/ecs-user/awiki-space/awiki-system-test
PYTHONDONTWRITEBYTECODE=1 uv run python -m py_compile tests_v2/id/test_identity_cli.py
AWIKI_CLI_UNDER_TEST=rust AWIKI_CLI_RUST_REPO=/home/ecs-user/awiki-space/awiki-cli-rs2 AWIKI_CLI_UPDATE_CACHE_ONLY=1 PYTHONDONTWRITEBYTECODE=1 uv run pytest -p no:cacheprovider tests_v2/id/test_identity_cli.py::test_id_bind_rust_migrates_legacy_config_json_before_identity_boundary -ra -q
```
...
- Rust formatting passed.
- Focused Rust bind migration selector passed: 1 passed, 0 failed, 16
  filtered out.
- Full Rust `identity_contract` passed: 17 passed, 0 failed.
- Rust package check passed.
- `xtask check-structure` passed with no undocumented Rust file over the
  1200-line soft cap.
- Go focused reference tests passed:
  `internal/cli` 11.486s, `internal/identity` 0.135s, and
  `internal/upgrade` 0.072s.
- Python syntax check for `tests_v2/id/test_identity_cli.py` passed.
- Focused `awiki-system-test` selector
  `test_id_bind_rust_migrates_legacy_config_json_before_identity_boundary`
  passed: 1 passed, 0 failed, 0 skipped in 5.28s.
- Dependency audit found existing Rustls/webpki/ring, base64/sha2/hmac, and
  approved `rusqlite`/`libsqlite3-sys` entries; no OpenSSL/native-tls stack or
  new dependency was added.
- Line counts after the slice: `app.rs` 1045,
  `identity_contract.rs` 1191, `test_identity_cli.py` 1125, and
  `tests_v2/id/CLAUDE.md` 13.
...
- The selector writes legacy `config.json`, removes `config.yaml`, then invokes
  real Rust `id bind --phone 13800138000` as the first CLI command after the
  legacy config is present. It does not require user-service, message-service,
  mail-service, listener, service-manager permissions, registered identities,
  JWT material, or live OTP state.

Coverage:

- Verifies valid-looking non-dry-run `id bind --phone 13800138000` migrates a
  legacy `config.json`-only workspace before returning the local
  no-active-identity `not_found` error.
- Verifies migrated `config.yaml` keeps legacy runtime mode, service base URL,
  and DID domain.
- Verifies `upgrade/meta.json` records workspace schema version `3`, a
  non-empty upgrade ID, and a backup directory containing `config.json.bak`
...
Boundary note: this is public CLI evidence for the local legacy
`config.json`-only migration branch in `id bind`, not live bind phone/email HTTP
behavior. It does not separately prove `id register`, `id replace-did`, legacy
SQLite import, imported/current k1 replacement RPCs, rollback, cleanup command
execution, or mail-service selectors. Mail-related system-test cases remain
deferred/gated and must not be counted as passed.
...
## 2026-05-18 Identity Profile Set Workspace Upgrade Hook

Timestamp: 2026-05-18T02:07:27+0800.

Scope: wire Go `internal/cli/id.go` `identityService()` workspace-upgrade
behavior into Rust `id profile set`. Go validates conflicting `--markdown` and
`--markdown-file` before constructing `identityService()`, then constructs the
service and migrates legacy `config.json` before dry-run planning or
`Service.SetProfile`.

Rust change:

- `crates/awiki-cli/src/app.rs`: `id profile set` now calls
  `resolve_config_for_workspace()` after markdown body-source conflict
  validation and before dry-run planning or live profile update.
- `crates/awiki-cli/tests/identity_profile_set_upgrade_contract.rs`: added a
  focused subprocess contract proving both ordering branches: conflict
  validation leaves legacy config untouched, while a valid profile-set request
  migrates legacy config before the local active-identity boundary.
- The new Rust test file is 204 lines, keeping the existing
  `identity_contract.rs` at 1182 lines and below the 1200-line soft cap.

System-test change:

- Added
  `tests_v2/id/test_identity_cli.py::test_id_profile_set_rust_migrates_legacy_config_json_before_identity_boundary`.
- Updated `tests_v2/id/CLAUDE.md` to mention the Rust `id profile set`
  legacy config workspace-upgrade coverage.
...
cargo +1.79.0 fmt
cargo +1.79.0 fmt --check
cargo +1.79.0 test -p awiki-cli --test identity_profile_set_upgrade_contract --locked
cargo +1.79.0 test -p awiki-cli --test identity_contract --locked
cargo +1.79.0 check -p awiki-cli --locked
cargo +1.79.0 run --bin xtask --locked -- check-structure
cargo +1.79.0 tree --workspace --locked | rg -i 'openssl|native-tls|openssl-sys|openssl-probe|openssl-src|reqwest|hyper|rustls|webpki|aws-lc|ring|libsqlite3-sys|sqlite|pkg-config|vcpkg|cc |systemd|dbus|launchd|kardianos|service-manager|tungstenite|websocket|serde_yaml|yaml|hmac|sha2|base64'
...
```text
cd /home/ecs-user/awiki-space/awiki-cli
go test ./internal/cli ./internal/identity ./internal/upgrade -run 'Test.*Profile|Test.*Identity|Test.*Config|TestUpgradeIfNeededMigratesLegacyConfigJSON|TestUpgradeIfNeededStampsCurrentWorkspaceMetadata' -count=1
```

```text
cd /home/ecs-user/awiki-space/awiki-system-test
PYTHONDONTWRITEBYTECODE=1 uv run python -m py_compile tests_v2/id/test_identity_cli.py
AWIKI_CLI_UNDER_TEST=rust AWIKI_CLI_RUST_REPO=/home/ecs-user/awiki-space/awiki-cli-rs2 AWIKI_CLI_UPDATE_CACHE_ONLY=1 PYTHONDONTWRITEBYTECODE=1 uv run pytest -p no:cacheprovider tests_v2/id/test_identity_cli.py::test_id_profile_set_rust_migrates_legacy_config_json_before_identity_boundary -ra -q
```

```text
cd /home/ecs-user/awiki-space/awiki-cli-rs2
wc -l crates/awiki-cli/src/app.rs crates/awiki-cli/tests/identity_contract.rs crates/awiki-cli/tests/identity_profile_set_upgrade_contract.rs docs/parity-matrix.md docs/verification/README.md
cd /home/ecs-user/awiki-space/awiki-system-test
wc -l tests_v2/id/test_identity_cli.py tests_v2/id/CLAUDE.md
```
...
- Rust formatting passed.
- Focused Rust profile-set upgrade contract passed: 2 passed, 0 failed.
- Full Rust `identity_contract` passed: 16 passed, 0 failed.
- Rust package check passed.
- `xtask check-structure` passed with no undocumented Rust file over the
  1200-line soft cap.
- Go focused reference tests passed:
  `internal/cli` 11.668s, `internal/identity` 0.423s, and
  `internal/upgrade` 0.363s.
- Python syntax check for `tests_v2/id/test_identity_cli.py` passed.
- Focused `awiki-system-test` selector
  `test_id_profile_set_rust_migrates_legacy_config_json_before_identity_boundary`
  passed: 1 passed, 0 failed, 0 skipped in 1.70s.
- Dependency audit found existing Rustls/webpki/ring, base64/sha2/hmac, and
  approved `rusqlite`/`libsqlite3-sys` entries; no OpenSSL/native-tls stack or
  new dependency was added.
- Line counts after the slice: `app.rs` 1045,
  `identity_contract.rs` 1182,
  `identity_profile_set_upgrade_contract.rs` 204,
  `test_identity_cli.py` 1179, and `tests_v2/id/CLAUDE.md` 13.
...
- `PYTHONDONTWRITEBYTECODE=1` and `pytest -p no:cacheprovider` avoid new
  Python cache artifacts.
- The selector writes legacy `config.json`, removes `config.yaml`, then invokes
  real Rust `id profile set --display-name Legacy Profile` as the first CLI
  ...
  registered identities, JWT material, or live OTP state.

Coverage:

- Verifies conflicting `--markdown` and `--markdown-file` validation remains
  before workspace upgrade and leaves legacy `config.json` untouched.
- Verifies valid `id profile set` migrates a legacy `config.json`-only
  workspace before returning the local no-active-identity `not_found` error.
- Verifies migrated `config.yaml` keeps legacy runtime mode, service base URL,
  and DID domain.
- Verifies `upgrade/meta.json` records workspace schema version `3`, a
  non-empty upgrade ID, and a backup directory containing `config.json.bak`
...
Boundary note: this is public CLI evidence for the local legacy
`config.json`-only migration branch in `id profile set`, not live profile
update HTTP behavior. It does not separately prove `id register`, `id bind`,
`id replace-did`, legacy SQLite import, imported/current k1 replacement RPCs,
rollback, cleanup command execution, or mail-service selectors. Mail-related
...
## 2026-05-18 Identity Refresh-Token Workspace Upgrade Hook

Timestamp: 2026-05-18T01:49:02+0800.

Scope: wire Go `internal/cli/id.go` `identityService()` workspace-upgrade
behavior into Rust `id refresh-token`. Go constructs `identityService()` before
dry-run planning or `Service.RefreshToken`, so Rust must migrate legacy
`config.json` before the local active-identity lookup boundary in non-dry-run
execution.

Rust change:

- `crates/awiki-cli/src/app.rs`: `id refresh-token` now calls
  `resolve_config_for_workspace()` before dry-run planning or live refresh.
- `crates/awiki-cli/tests/identity_contract.rs`: added
  `identity_refresh_token_migrates_legacy_config_json_before_active_identity_boundary_like_go`.
- Shared empty-identity-boundary assertions cover both `id profile get` and
  `id refresh-token` while keeping `identity_contract.rs` below the 1200-line
  soft cap.
...
- Updated `tests_v2/id/CLAUDE.md` to mention the Rust `id refresh-token`
  legacy config workspace-upgrade coverage.
...
cargo +1.79.0 fmt
cargo +1.79.0 fmt --check
cargo +1.79.0 test -p awiki-cli --test identity_contract identity_refresh_token_migrates_legacy_config_json_before_active_identity_boundary_like_go --locked
cargo +1.79.0 test -p awiki-cli --test identity_contract --locked
cargo +1.79.0 check -p awiki-cli --locked
cargo +1.79.0 run --bin xtask --locked -- check-structure
cargo +1.79.0 tree --workspace --locked | rg -i 'openssl|native-tls|openssl-sys|openssl-probe|openssl-src|reqwest|hyper|rustls|webpki|aws-lc|ring|libsqlite3-sys|sqlite|pkg-config|vcpkg|cc |systemd|dbus|launchd|kardianos|service-manager|tungstenite|websocket|serde_yaml|yaml|hmac|sha2|base64'
...
```text
cd /home/ecs-user/awiki-space
wc -l awiki-cli-rs2/crates/awiki-cli/src/app.rs awiki-cli-rs2/crates/awiki-cli/tests/identity_contract.rs awiki-cli-rs2/docs/parity-matrix.md awiki-cli-rs2/docs/verification/README.md awiki-system-test/tests_v2/id/test_identity_cli.py awiki-system-test/tests_v2/id/CLAUDE.md
```

```text
cd /home/ecs-user/awiki-space/awiki-cli
go test ./internal/cli ./internal/identity ./internal/upgrade -run 'Test.*Refresh|Test.*Identity|Test.*Config|TestUpgradeIfNeededMigratesLegacyConfigJSON|TestUpgradeIfNeededStampsCurrentWorkspaceMetadata' -count=1
```

```text
cd /home/ecs-user/awiki-space/awiki-system-test
PYTHONDONTWRITEBYTECODE=1 uv run python -m py_compile tests_v2/id/test_identity_cli.py
AWIKI_CLI_UNDER_TEST=rust AWIKI_CLI_RUST_REPO=/home/ecs-user/awiki-space/awiki-cli-rs2 AWIKI_CLI_UPDATE_CACHE_ONLY=1 PYTHONDONTWRITEBYTECODE=1 uv run pytest -p no:cacheprovider tests_v2/id/test_identity_cli.py::test_id_refresh_token_rust_migrates_legacy_config_json_before_identity_boundary -ra -q
```

Observed results:

- Rust formatting passed.
- Focused Rust refresh-token migration selector passed: 1 passed, 0 failed, 15
  filtered out.
- Full Rust `identity_contract` passed: 16 passed, 0 failed.
- Rust package check passed.
- `xtask check-structure` passed with no undocumented Rust file over the
  1200-line soft cap. Line counts after the slice: `app.rs` 1045 lines and
  `identity_contract.rs` 1182 lines.
- Go focused reference tests passed:
  `internal/cli` 23.904s, `internal/identity` 0.119s, and
  `internal/upgrade` 0.063s.
- Python syntax check for `tests_v2/id/test_identity_cli.py` passed.
- Focused `awiki-system-test` selector
  `test_id_refresh_token_rust_migrates_legacy_config_json_before_identity_boundary`
  passed: 1 passed, 0 failed, 0 skipped in 0.23s.
- Dependency audit found existing Rustls/webpki/ring, base64/sha2/hmac, and
  approved `rusqlite`/`libsqlite3-sys` entries; no OpenSSL/native-tls stack or
  new dependency was added.
...
- `PYTHONDONTWRITEBYTECODE=1` and `pytest -p no:cacheprovider` avoid new
  Python cache artifacts.
- The selector writes legacy `config.json`, removes `config.yaml`, then invokes
  real Rust `id refresh-token` as the first CLI command after the legacy config
  is present. It does not require user-service, message-service, mail-service,
  listener, service-manager permissions, registered identities, JWT material,
  or live OTP state.

Coverage:

- Verifies `id refresh-token` migrates a legacy `config.json`-only workspace
  before returning the local no-active-identity `not_found` error.
- Verifies migrated `config.yaml` keeps legacy runtime mode, service base URL,
  and DID domain.
- Verifies `upgrade/meta.json` records workspace schema version `3`, a
  non-empty upgrade ID, and a backup directory containing `config.json.bak`
...
Boundary note: this is public CLI evidence for the local legacy
`config.json`-only migration branch in `id refresh-token`, not live JWT refresh
HTTP behavior. It does not separately prove `id register`, `id bind`,
`id profile set`, `id replace-did`, legacy SQLite import, imported/current k1
replacement RPCs, rollback, cleanup command execution, or mail-service
selectors. Mail-related system-test cases remain deferred/gated and must not be
counted as passed.
...
## 2026-05-18 Identity Profile Get Workspace Upgrade Hook

Timestamp: 2026-05-18T01:42:57+0800.

Scope: wire Go `internal/cli/id.go` `identityService()` workspace-upgrade
behavior into Rust `id profile get`. Go `Service.GetProfile` defaults a
missing target to self-profile lookup after `identityService()` is constructed,
so Rust must migrate legacy `config.json` before the local no-active-identity
boundary.

Rust change:

- `crates/awiki-cli/src/app.rs`: `id profile get` now calls
  `resolve_config_for_workspace()` before invoking `identity::get_profile`.
- `crates/awiki-cli/tests/identity_contract.rs`: added
  `identity_profile_get_migrates_legacy_config_json_before_self_identity_boundary_like_go`.
- Shared legacy-config/runtime-state assertions were reused to keep
  `identity_contract.rs` below the 1200-line soft cap.

System-test change:

- Added
  `tests_v2/id/test_identity_cli.py::test_id_profile_get_rust_migrates_legacy_config_json_before_self_identity_boundary`.
- Updated `tests_v2/id/CLAUDE.md` to mention the Rust `id profile get` legacy
...
cargo +1.79.0 fmt
cargo +1.79.0 fmt --check
cargo +1.79.0 test -p awiki-cli --test identity_contract identity_profile_get_migrates_legacy_config_json_before_self_identity_boundary_like_go --locked
cargo +1.79.0 test -p awiki-cli --test identity_contract --locked
cargo +1.79.0 check -p awiki-cli --locked
cargo +1.79.0 run --bin xtask --locked -- check-structure
cargo +1.79.0 tree --workspace --locked | rg -i 'openssl|native-tls|openssl-sys|openssl-probe|openssl-src|reqwest|hyper|rustls|webpki|aws-lc|ring|libsqlite3-sys|sqlite|pkg-config|vcpkg|cc |systemd|dbus|launchd|kardianos|service-manager|tungstenite|websocket|serde_yaml|yaml|hmac|sha2|base64'
...
cd /home/ecs-user/awiki-space/awiki-system-test
PYTHONDONTWRITEBYTECODE=1 uv run python -m py_compile tests_v2/id/test_identity_cli.py
AWIKI_CLI_UNDER_TEST=rust AWIKI_CLI_RUST_REPO=/home/ecs-user/awiki-space/awiki-cli-rs2 AWIKI_CLI_UPDATE_CACHE_ONLY=1 PYTHONDONTWRITEBYTECODE=1 uv run pytest -p no:cacheprovider tests_v2/id/test_identity_cli.py::test_id_profile_get_rust_migrates_legacy_config_json_before_self_identity_boundary -ra -q
```

Observed results:

- Rust formatting passed.
- Focused Rust profile-get migration selector passed: 1 passed, 0 failed, 14
  filtered out.
- Full Rust `identity_contract` passed: 15 passed, 0 failed.
- Rust package check passed.
- `xtask check-structure` passed with no undocumented Rust file over the
  1200-line soft cap. Line counts after the slice: `app.rs` 1045 lines and
  `identity_contract.rs` 1167 lines.
- Go focused reference tests passed:
  `internal/cli` 13.183s, `internal/identity` 0.137s, and
  `internal/upgrade` 0.078s.
- Python syntax check for `tests_v2/id/test_identity_cli.py` passed.
- Focused `awiki-system-test` selector
  `test_id_profile_get_rust_migrates_legacy_config_json_before_self_identity_boundary`
...
- `PYTHONDONTWRITEBYTECODE=1` and `pytest -p no:cacheprovider` avoid new
  Python cache artifacts.
- The selector writes legacy `config.json`, removes `config.yaml`, then invokes
  real Rust `id profile get` without `--self`, `--handle`, or `--did` as the
  first CLI command after the legacy config is present. It does not require
  user-service, message-service, mail-service, listener, service-manager
  permissions, registered identities, or live OTP state.

Coverage:

- Verifies `id profile get` migrates a legacy `config.json`-only workspace
  before returning the local no-active-identity `not_found` error.
- Verifies migrated `config.yaml` keeps legacy runtime mode, service base URL,
  and DID domain.
- Verifies `upgrade/meta.json` records workspace schema version `3`, a
  ...
  equal to the seeded legacy config.
- Verifies `upgrade/upgrade_journal.json` is cleared after success.
- Verifies the self-profile identity boundary does not create SQLite DB state,
  listener pid files, or runtime socket artifacts.

Boundary note: this is public CLI evidence for the local legacy
`config.json`-only migration branch in `id profile get` default self-profile
mode. It does not separately prove `id profile set`, handle/did profile remote
lookup, `id register`, `id bind`, `id refresh-token`, `id replace-did`, legacy
SQLite import, imported/current k1 replacement RPCs, rollback, cleanup command
execution, or mail-service selectors. Mail-related system-test cases remain
deferred/gated and must not be counted as passed.
...
Scope: wire Go `internal/cli/id.go` `identityService()` workspace-upgrade
behavior into Rust `id use` while preserving Go's argument-count validation
before service construction.

Rust change:

- `crates/awiki-cli/src/app.rs`: `id use` now rejects missing/extra identity
  arguments before calling `resolve_config_for_workspace()`. This matches Go
  `runIDUse`, which checks `len(args) != 1` before constructing the service
  through `identityService()`.
- `crates/awiki-cli/tests/identity_contract.rs`: added
  `identity_use_validates_argument_before_workspace_upgrade_like_go` and
  `identity_use_migrates_legacy_config_json_before_switch_like_go`.
- The identity contract file was also deduplicated with shared legacy-config
  upgrade assertion helpers to keep it below the 1200-line soft cap.

System-test change:

- Added
  `tests_v2/id/test_identity_cli.py::test_id_use_rust_migrates_legacy_config_json_before_switch`.
- Updated `tests_v2/id/CLAUDE.md` to mention the Rust `id use` legacy config
  workspace-upgrade coverage.
...
cargo +1.79.0 fmt --check
cargo +1.79.0 check -p awiki-cli --locked
cargo +1.79.0 test -p awiki-cli --test identity_contract identity_use_validates_argument_before_workspace_upgrade_like_go --locked
cargo +1.79.0 test -p awiki-cli --test identity_contract identity_use_migrates_legacy_config_json_before_switch_like_go --locked
cargo +1.79.0 test -p awiki-cli --test identity_contract --locked
cargo +1.79.0 run --bin xtask --locked -- check-structure
cargo +1.79.0 tree --workspace --locked | rg -i 'openssl|native-tls|openssl-sys|openssl-probe|openssl-src|reqwest|hyper|rustls|webpki|aws-lc|ring|libsqlite3-sys|sqlite|pkg-config|vcpkg|cc |systemd|dbus|launchd|kardianos|service-manager|tungstenite|websocket|serde_yaml|yaml|hmac|sha2|base64'
wc -l crates/awiki-cli/src/app.rs crates/awiki-cli/tests/identity_contract.rs docs/parity-matrix.md docs/verification/README.md
...
```text
cd /home/ecs-user/awiki-space/awiki-cli
go test ./internal/cli ./internal/identity ./internal/upgrade -run 'Test.*Use|Test.*Identity|Test.*Config|TestUpgradeIfNeededMigratesLegacyConfigJSON|TestUpgradeIfNeededStampsCurrentWorkspaceMetadata' -count=1
```

```text
cd /home/ecs-user/awiki-space/awiki-system-test
PYTHONDONTWRITEBYTECODE=1 uv run python -m py_compile tests_v2/id/test_identity_cli.py
AWIKI_CLI_UNDER_TEST=rust AWIKI_CLI_RUST_REPO=/home/ecs-user/awiki-space/awiki-cli-rs2 AWIKI_CLI_UPDATE_CACHE_ONLY=1 PYTHONDONTWRITEBYTECODE=1 uv run pytest -p no:cacheprovider tests_v2/id/test_identity_cli.py::test_id_use_rust_migrates_legacy_config_json_before_switch -ra -q
wc -l tests_v2/id/test_identity_cli.py tests_v2/id/CLAUDE.md
```
...
- Focused Rust validation selector passed: 1 passed, 0 failed.
- Focused Rust valid-switch migration selector passed: 1 passed, 0 failed.
- Full Rust `identity_contract` passed: 13 passed, 0 failed.
- `xtask check-structure` passed with no undocumented Rust file over the
  1200-line soft cap. Line counts after the slice: `app.rs` 1045 lines and
  `identity_contract.rs` 1175 lines.
- Go focused reference tests passed:
  `internal/cli` 35.114s, `internal/identity` 0.123s, and
  `internal/upgrade` 0.066s.
- Python syntax check for `tests_v2/id/test_identity_cli.py` passed.
- Focused `awiki-system-test` selector:
  `test_id_use_rust_migrates_legacy_config_json_before_switch` passed:
  1 passed, 0 failed, 0 skipped in 6.81s.
- Dependency audit found existing Rustls/webpki/ring, base64/sha2/hmac, and
  approved `rusqlite`/`libsqlite3-sys` entries; no OpenSSL/native-tls stack or
  new dependency was added.
...
- `PYTHONDONTWRITEBYTECODE=1` and `pytest -p no:cacheprovider` avoid new
  Python cache artifacts.
- The new selector seeds the local identity store directly, writes legacy
  `config.json`, then invokes real Rust `id use bob` as the first CLI command
  after the legacy config is present. It does not require user-service,
  message-service, mail-service, listener, service-manager permissions, or live
  OTP state.

Coverage:

- Verifies missing `id use <identity>` validation happens before workspace
  upgrade by leaving legacy `config.json` untouched and avoiding `config.yaml`
  and `upgrade/meta.json`.
- Verifies valid `id use bob` migrates a legacy `config.json`-only workspace
  before changing the identity-index default.
- Verifies `id current` returns `bob` after the switch.
- Verifies migrated `config.yaml` keeps legacy runtime mode, service base URL,
  DID domain, and the old `identity.active` value. This is intentional parity
  with Go `Manager.SetDefault`, which updates the identity index default rather
  than rewriting `config.yaml`.
- Verifies `upgrade/meta.json` records workspace schema version `3`, a
  non-empty upgrade ID, and a backup directory containing `config.json.bak`
  equal to the seeded legacy config.
- Verifies `upgrade/upgrade_journal.json` is cleared after success.
- Verifies the local use branch does not create SQLite DB state, listener pid
  files, or runtime socket artifacts.

Boundary note: this is public CLI evidence for the local legacy
`config.json`-only migration branch in `id use`, plus Rust subprocess evidence
for Go argument-count validation ordering. It does not separately prove legacy
config migration for `id register`, `id bind`, `id refresh-token`, `id profile`,
`id resolve`, `id replace-did`, legacy SQLite import, imported/current k1
replacement RPCs, rollback, cleanup command execution, or mail-service
selectors. Mail-related system-test cases remain deferred/gated and must not be
counted as passed.
...
## 2026-05-18 Identity Create Workspace Upgrade Hook

Timestamp: 2026-05-18T01:02:50+0800.

Scope: wire Go `internal/cli/id.go` `identityService()` workspace-upgrade
behavior into Rust `id create` while preserving Go's validation-before-service
ordering for a missing `--name`.

Rust change:

- `crates/awiki-cli/src/app.rs`: `id create` now reads `--name` and
  `--identity`, rejects missing `--name`, and only then calls
  `resolve_config_for_workspace()`. This matches Go `runIDCreate`, which reads
  flags, validates `--name`, and then constructs the service through
  `identityService()`.
- `crates/awiki-cli/tests/identity_contract.rs`: added
  `identity_create_validates_name_before_workspace_upgrade_like_go` and
  `identity_create_migrates_legacy_config_json_before_create_like_go`.

System-test change:

- Added
  `tests_v2/id/test_identity_cli.py::test_id_create_rust_migrates_legacy_config_json_before_create`.
- Updated `tests_v2/id/CLAUDE.md` to mention the Rust `id create` legacy config
  workspace-upgrade coverage.
...
cargo +1.79.0 fmt --check
cargo +1.79.0 check -p awiki-cli --locked
cargo +1.79.0 test -p awiki-cli --test identity_contract identity_create_validates_name_before_workspace_upgrade_like_go --locked
cargo +1.79.0 test -p awiki-cli --test identity_contract identity_create_migrates_legacy_config_json_before_create_like_go --locked
cargo +1.79.0 test -p awiki-cli --test identity_contract --locked
cargo +1.79.0 run --bin xtask --locked -- check-structure
cargo +1.79.0 tree --workspace --locked | rg -i 'openssl|native-tls|openssl-sys|openssl-probe|openssl-src|reqwest|hyper|rustls|webpki|aws-lc|ring|libsqlite3-sys|sqlite|pkg-config|vcpkg|cc |systemd|dbus|launchd|kardianos|service-manager|tungstenite|websocket|serde_yaml|yaml|hmac|sha2|base64'
wc -l crates/awiki-cli/src/app.rs crates/awiki-cli/tests/identity_contract.rs docs/parity-matrix.md docs/verification/README.md
...
```text
cd /home/ecs-user/awiki-space/awiki-cli
go test ./internal/cli ./internal/identity ./internal/upgrade -run 'Test.*Create|Test.*Identity|Test.*Config|TestUpgradeIfNeededMigratesLegacyConfigJSON|TestUpgradeIfNeededStampsCurrentWorkspaceMetadata' -count=1
```

```text
cd /home/ecs-user/awiki-space/awiki-system-test
PYTHONDONTWRITEBYTECODE=1 uv run python -m py_compile tests_v2/id/test_identity_cli.py
AWIKI_CLI_UNDER_TEST=rust AWIKI_CLI_RUST_REPO=/home/ecs-user/awiki-space/awiki-cli-rs2 AWIKI_CLI_UPDATE_CACHE_ONLY=1 PYTHONDONTWRITEBYTECODE=1 uv run pytest -p no:cacheprovider tests_v2/id/test_identity_cli.py::test_id_create_rust_migrates_legacy_config_json_before_create -ra -q
wc -l tests_v2/id/test_identity_cli.py tests_v2/id/CLAUDE.md
```
...
- Rust formatting and package check passed.
- Focused Rust validation selector passed: 1 passed, 0 failed.
- Focused Rust valid-create migration selector passed: 1 passed, 0 failed.
- Full Rust `identity_contract` passed: 11 passed, 0 failed.
- `xtask check-structure` passed with no undocumented Rust file over the
  1200-line soft cap. Line counts after the slice: `app.rs` 1045 lines and
  `identity_contract.rs` 1061 lines.
- Go focused reference tests passed:
  `internal/cli` 15.395s, `internal/identity` 0.152s, and
  `internal/upgrade` 0.104s.
- Python syntax check for `tests_v2/id/test_identity_cli.py` passed.
- Focused `awiki-system-test` selector:
  `test_id_create_rust_migrates_legacy_config_json_before_create` passed:
  1 passed, 0 failed, 0 skipped in 7.40s.
- Dependency audit found existing Rustls/webpki/ring, base64/sha2/hmac, and
  approved `rusqlite`/`libsqlite3-sys` entries; no OpenSSL/native-tls stack or
  new dependency was added.
...
- `PYTHONDONTWRITEBYTECODE=1` and `pytest -p no:cacheprovider` avoid new
  Python cache artifacts.
- The new selector invokes real Rust `id create --name Legacy Create
  --identity legacy-create` as the first CLI command after writing legacy
  `config.json`. It does not require user-service, message-service,
  mail-service, listener, service-manager permissions, or live OTP state.

Coverage:

- Verifies missing `id create --name` validation happens before workspace
  upgrade by leaving legacy `config.json` untouched and avoiding `config.yaml`
  and `upgrade/meta.json`.
- Verifies valid `id create` migrates a legacy `config.json`-only workspace
  before local identity creation.
- Verifies the generated DID uses the migrated DID domain and the created
  identity becomes the current identity.
- Verifies migrated `config.yaml` keeps legacy runtime mode, service base URL,
  and DID domain.
- Verifies `upgrade/meta.json` records workspace schema version `3`, a
  non-empty upgrade ID, and a backup directory containing `config.json.bak`
  equal to the seeded legacy config.
- Verifies `upgrade/upgrade_journal.json` is cleared after success.
- Verifies the local create branch does not create SQLite DB state, listener pid
  files, or runtime socket artifacts.

Boundary note: this is public CLI evidence for the local legacy
`config.json`-only migration branch in `id create`, plus Rust subprocess
evidence for Go validation ordering. It does not separately prove legacy config
migration for `id use`, `id register`, `id bind`, `id refresh-token`,
`id profile`, `id resolve`, `id replace-did`, legacy SQLite import,
imported/current k1 replacement RPCs, rollback, cleanup command execution, or
mail-service selectors. Mail-related system-test cases remain deferred/gated
and must not be counted as passed.
...
## 2026-05-18 Identity Import-v1 Workspace Upgrade Hook

Timestamp: 2026-05-18T00:53:28+0800.

Scope: wire Go `internal/cli/id.go` `identityService()` workspace-upgrade
behavior into Rust `id import-v1`, then prove a local legacy `config.json`
migration occurs before flat v1 legacy credential import.

Rust change:

- `crates/awiki-cli/src/app.rs`: `id import-v1` now calls
  `resolve_config_for_workspace()` where it previously called
  `resolve_config()`. This matches Go `runIDImportV1`, which reads flags and
  then constructs the service through `identityService()`;
  `identityService()` calls `resolveConfigForWorkspace()` before legacy import
  execution.
- `crates/awiki-cli/tests/identity_contract.rs`: added
  `identity_import_v1_migrates_legacy_config_json_before_import_like_go`.

System-test change:

- Added
  `tests_v2/id/test_identity_cli.py::test_id_import_v1_rust_migrates_legacy_config_json_before_import`.
- Updated `tests_v2/id/CLAUDE.md` to mention the Rust `id import-v1` legacy
  config workspace-upgrade coverage.
...
cargo +1.79.0 fmt --check
cargo +1.79.0 check -p awiki-cli --locked
cargo +1.79.0 test -p awiki-cli --test identity_contract identity_import_v1_migrates_legacy_config_json_before_import_like_go --locked
cargo +1.79.0 test -p awiki-cli --test identity_contract --locked
cargo +1.79.0 run --bin xtask --locked -- check-structure
cargo +1.79.0 tree --workspace --locked | rg -i 'openssl|native-tls|openssl-sys|openssl-probe|openssl-src|reqwest|hyper|rustls|webpki|aws-lc|ring|libsqlite3-sys|sqlite|pkg-config|vcpkg|cc |systemd|dbus|launchd|kardianos|service-manager|tungstenite|websocket|serde_yaml|yaml|hmac|sha2|base64'
wc -l crates/awiki-cli/src/app.rs crates/awiki-cli/tests/identity_contract.rs docs/parity-matrix.md docs/verification/README.md
...
```text
cd /home/ecs-user/awiki-space/awiki-cli
go test ./internal/cli ./internal/identity ./internal/upgrade -run 'Test.*Import|Test.*Identity|Test.*Config|TestUpgradeIfNeededMigratesLegacyConfigJSON|TestUpgradeIfNeededStampsCurrentWorkspaceMetadata' -count=1
```

```text
cd /home/ecs-user/awiki-space/awiki-system-test
PYTHONDONTWRITEBYTECODE=1 uv run python -m py_compile tests_v2/id/test_identity_cli.py
AWIKI_CLI_UNDER_TEST=rust AWIKI_CLI_RUST_REPO=/home/ecs-user/awiki-space/awiki-cli-rs2 AWIKI_CLI_UPDATE_CACHE_ONLY=1 PYTHONDONTWRITEBYTECODE=1 uv run pytest -p no:cacheprovider tests_v2/id/test_identity_cli.py::test_id_import_v1_rust_migrates_legacy_config_json_before_import -ra -q
wc -l tests_v2/id/test_identity_cli.py tests_v2/id/CLAUDE.md
```
...
- Rust formatting and package check passed.
- Focused Rust `identity_contract` selector passed: 1 passed, 0 failed.
- Full Rust `identity_contract` passed: 9 passed, 0 failed.
- `xtask check-structure` passed with no undocumented Rust file over the
  1200-line soft cap. Line counts after the slice: `app.rs` 1045 lines and
  `identity_contract.rs` 912 lines.
- Go focused reference tests passed:
  `internal/cli` 21.658s, `internal/identity` 0.136s, and
  `internal/upgrade` 0.363s.
- Python syntax check for `tests_v2/id/test_identity_cli.py` passed.
- Focused `awiki-system-test` selector:
  `test_id_import_v1_rust_migrates_legacy_config_json_before_import` passed:
  1 passed, 0 failed, 0 skipped in 0.38s.
- Dependency audit found existing Rustls/webpki/ring, base64/sha2/hmac, and
  approved `rusqlite`/`libsqlite3-sys` entries; no OpenSSL/native-tls stack or
  new dependency was added.
...
- `PYTHONDONTWRITEBYTECODE=1` and `pytest -p no:cacheprovider` avoid new
  Python cache artifacts.
- The new selector invokes real Rust `id import-v1 --name legacy-flat` as the
  first CLI command after writing legacy `config.json` and seeding a local flat
  v1 legacy identity. It does not require user-service, message-service,
  mail-service, listener, service-manager permissions, or live OTP state.

Coverage:

- Verifies `id import-v1` migrates a legacy `config.json`-only workspace before
  importing a flat v1 legacy identity.
- Verifies the imported identity is persisted and becomes the current identity.
- Verifies migrated `config.yaml` keeps legacy runtime mode, service base URL,
  and DID domain.
- Verifies `upgrade/meta.json` records workspace schema version `3`, a
  non-empty upgrade ID, and a backup directory containing `config.json.bak`
  equal to the seeded legacy config.
- Verifies `upgrade/upgrade_journal.json` is cleared after success.
- Verifies the import branch does not create listener pid files or runtime
  socket artifacts.

Boundary note: this is public CLI evidence for the local legacy
`config.json`-only migration branch in `id import-v1`. It does not separately
prove legacy config migration for `id use`, `id create`, `id register`,
`id bind`, `id refresh-token`, `id profile`, `id resolve`, `id replace-did`,
legacy SQLite import, imported/current k1 replacement RPCs, rollback, cleanup
command execution, or mail-service selectors. Mail-related system-test cases
remain deferred/gated and must not be counted as passed.
...
## 2026-05-18 Identity Read Workspace Upgrade Hook

Timestamp: 2026-05-18T00:37:25+0800.

Scope: wire Go `internal/cli/id.go` `identityService()` workspace-upgrade
behavior into Rust local identity read commands, then prove a local legacy
`config.json` migration through `id status` before the command reads identity
store state.

Rust change:

- `crates/awiki-cli/src/app.rs`: `id status`, `id list`, and `id current` now
  call `resolve_config_for_workspace()` where they previously called
  `resolve_config()`. This matches Go `runIDStatus`, `runIDList`, and
  `runIDCurrent`, which obtain their service through `identityService()`, and
  `identityService()` calls `resolveConfigForWorkspace()` before constructing
  the identity service.
- `crates/awiki-cli/tests/identity_contract.rs`: added
  `identity_status_migrates_legacy_config_json_before_store_read_like_go`.

System-test change:

- Added
  `tests_v2/id/test_identity_cli.py::test_id_status_rust_migrates_legacy_config_json_before_store_read`.
- Updated `tests_v2/id/CLAUDE.md` to mention the Rust `id status` legacy config
  workspace-upgrade coverage.
...
cargo +1.79.0 fmt --check
cargo +1.79.0 check -p awiki-cli --locked
cargo +1.79.0 test -p awiki-cli --test identity_contract identity_status_migrates_legacy_config_json_before_store_read_like_go --locked
cargo +1.79.0 test -p awiki-cli --test identity_contract --locked
cargo +1.79.0 run --bin xtask --locked -- check-structure
cargo +1.79.0 tree --workspace --locked | rg -i 'openssl|native-tls|openssl-sys|openssl-probe|openssl-src|reqwest|hyper|rustls|webpki|aws-lc|ring|libsqlite3-sys|sqlite|pkg-config|vcpkg|cc |systemd|dbus|launchd|kardianos|service-manager|tungstenite|websocket|serde_yaml|yaml|hmac|sha2|base64'
```
...
```text
cd /home/ecs-user/awiki-space/awiki-cli
go test ./internal/cli ./internal/identity ./internal/upgrade -run 'Test.*Status|Test.*List|Test.*Current|Test.*Identity|Test.*Config|TestUpgradeIfNeededMigratesLegacyConfigJSON|TestUpgradeIfNeededStampsCurrentWorkspaceMetadata' -count=1
```

```text
cd /home/ecs-user/awiki-space/awiki-system-test
PYTHONDONTWRITEBYTECODE=1 uv run python -m py_compile tests_v2/id/test_identity_cli.py
AWIKI_CLI_UNDER_TEST=rust AWIKI_CLI_RUST_REPO=/home/ecs-user/awiki-space/awiki-cli-rs2 AWIKI_CLI_UPDATE_CACHE_ONLY=1 PYTHONDONTWRITEBYTECODE=1 uv run pytest -p no:cacheprovider tests_v2/id/test_identity_cli.py::test_id_status_rust_migrates_legacy_config_json_before_store_read -ra -q
```
...
- Rust formatting and package check passed.
- Focused Rust `identity_contract` selector passed: 1 passed, 0 failed.
- Full Rust `identity_contract` passed: 8 passed, 0 failed.
- `xtask check-structure` passed with no undocumented Rust file over the
  1200-line soft cap.
- Go focused reference tests passed:
  `internal/cli` 15.726s, `internal/identity` 0.132s, and
  `internal/upgrade` 0.078s.
- Python syntax check for `tests_v2/id/test_identity_cli.py` passed.
- Focused `awiki-system-test` selector:
  `test_id_status_rust_migrates_legacy_config_json_before_store_read` passed:
  1 passed, 0 failed, 0 skipped in 0.36s.
- Dependency audit found existing Rustls/webpki/ring, base64/sha2/hmac, and
  approved `rusqlite`/`libsqlite3-sys` entries; no OpenSSL/native-tls stack or
  new dependency was added.
...
- `PYTHONDONTWRITEBYTECODE=1` and `pytest -p no:cacheprovider` avoid new
  Python cache artifacts.
- The new selector invokes real Rust `id status` as the first CLI command
  after writing legacy `config.json`. It does not require user-service,
  message-service, mail-service, listener, service-manager permissions, or
  registered identities.

Coverage:

- Verifies `id status` migrates a legacy `config.json`-only workspace before
...
- Verifies `upgrade/meta.json` records workspace schema version `3`, a
  ...
  equal to the seeded legacy config.
- Verifies `upgrade/upgrade_journal.json` is cleared after success.
- Verifies the empty-store `id status` branch does not create SQLite DB state,
  listener pid files, or runtime socket artifacts.

Boundary note: this is public CLI evidence for the local legacy
`config.json`-only migration branch in `id status`, plus direct Rust code
coverage that `id list` and `id current` use the same workspace resolver
boundary. It does not separately prove legacy config migration for `id use`,
`id create`, `id register`, `id bind`, `id refresh-token`, `id profile`,
`id resolve`, `id import-v1`, `id recover`, `id replace-did`, legacy SQLite
import, imported/current k1 replacement RPCs, rollback, cleanup command
execution, or mail-service selectors. Mail-related system-test cases remain
deferred/gated and must not be counted as passed.

Dependency note: no Rust dependency was added. Cargo manifests and lockfile are
unchanged; the slice reuses existing local upgrade/config/identity machinery
and keeps the Rustls-first / approved `rusqlite + bundled` dependency policy
unchanged.

## 2026-05-18 Site Workspace Upgrade Hook

Timestamp: 2026-05-18T00:09:43+0800.

Scope: wire Go `internal/cli/site.go` `siteService()` workspace-upgrade
behavior into Rust site handlers, then prove a local legacy `config.json`
migration through `site page list` before the command reaches the
no-active-identity site boundary.

Rust change:

- `crates/awiki-cli/src/app/site_handlers.rs`: `site root get`,
  `site root set`, `site page list`, `site page get`, `site page create`,
  `site page update`, `site page rename`, and `site page delete` now call
  `resolve_config_for_workspace()` where they previously called
  `resolve_config()`. This matches Go `siteService()`, which calls
  `resolveConfigForWorkspace()` before constructing the site service.
- `crates/awiki-cli/tests/site_contract.rs`: added
  `site_page_list_migrates_legacy_config_json_before_identity_boundary_like_go`.
  ...
  cannot leak into the site no-active-identity contract.

System-test change:

- Added
  `tests_v2/site/test_site_cli.py::test_site_page_list_rust_migrates_legacy_config_json_before_identity_boundary`.
- Updated `tests_v2/site/CLAUDE.md` to mention the Rust `site page list`
  legacy config workspace-upgrade coverage.
...
cargo +1.79.0 fmt --check
cargo +1.79.0 check -p awiki-cli --locked
cargo +1.79.0 test -p awiki-cli --test site_contract site_page_list_migrates_legacy_config_json_before_identity_boundary_like_go --locked
...
```text
cd /home/ecs-user/awiki-space/awiki-cli
go test ./internal/cli ./internal/site ./internal/upgrade -run 'Test.*Site|Test.*Config|TestUpgradeIfNeededMigratesLegacyConfigJSON|TestUpgradeIfNeededStampsCurrentWorkspaceMetadata' -count=1
```

```text
cd /home/ecs-user/awiki-space/awiki-system-test
PYTHONDONTWRITEBYTECODE=1 uv run python -m py_compile tests_v2/site/test_site_cli.py
AWIKI_CLI_UNDER_TEST=rust AWIKI_CLI_RUST_REPO=/home/ecs-user/awiki-space/awiki-cli-rs2 AWIKI_CLI_UPDATE_CACHE_ONLY=1 PYTHONDONTWRITEBYTECODE=1 uv run pytest -p no:cacheprovider tests_v2/site/test_site_cli.py::test_site_page_list_rust_migrates_legacy_config_json_before_identity_boundary -ra -q
```

Observed results:

- Rust formatting passed.
- Focused Rust `site_contract` selector passed: 1 passed, 0 failed.
- Full Rust `site_contract` passed: 6 passed, 0 failed.
- Rust package check passed.
- `xtask check-structure` passed with no undocumented Rust file over the
  1200-line soft cap.
- Go focused reference tests passed:
  `internal/cli` 16.452s, `internal/site` 0.012s, and
  `internal/upgrade` 0.060s.
- Python syntax check for `tests_v2/site/test_site_cli.py` passed.
- Focused `awiki-system-test` selector:
  `test_site_page_list_rust_migrates_legacy_config_json_before_identity_boundary`
  passed: 1 passed, 0 failed, 0 skipped in 0.20s.
...
- The new selector uses an isolated local workspace and invokes real Rust
  `site page list` as the first CLI command after writing legacy `config.json`.
  It does not require user-service, message-service, site-service, mail-service,
  listener, service-manager permissions, or registered identities.

Coverage:

- Rust site handlers now follow Go `siteService()` config boundary: raw config
  resolve, `workspace_upgrade` trace phase, `upgrade_if_needed`, raw resolve
  refresh, then dry-run render or site service execution.
- Verifies `site page list` migrates a legacy `config.json`-only workspace
  before failing at the no-active-identity boundary.
- Verifies migrated `config.yaml` keeps legacy runtime mode, service base URL,
  and DID domain.
- Verifies `upgrade/meta.json` records workspace schema version `3`, a
  non-empty upgrade ID, and a backup directory containing `config.json.bak`
...
Boundary note: this is public CLI evidence for the local legacy
`config.json`-only migration branch in `site page list`, plus direct Rust code
coverage that all currently translated site handlers use the same workspace
resolver boundary. It does not separately subprocess-test legacy config
migration for every site subcommand, run live tenant-admin site CRUD in this
local selector, prove legacy SQLite import through workspace migration, legacy
identity import, imported/current k1 replacement RPCs, rollback, platform
cleanup command execution, all-command upgrade-hook coverage, or mail-service
selectors.

Dependency note: no Rust dependency was added. Cargo manifests and lockfile are
unchanged; the slice reuses existing local upgrade/config/site machinery and
...
Scope: add direct `awiki-system-test` subprocess coverage for explicit
`msg secure init --with <peer>` production execution. This slice proves the
manual secure-init command path itself; it does not promote `msg secure retry`,
`msg secure repair`, WebSocket/local bridge secure execution, runtime listener
live `ProcessIncoming`, secure attachments, mail, or full repository-wide
system acceptance.
...
AWIKI_CLI_UPDATE_CACHE_ONLY=1 \
PYTHONDONTWRITEBYTECODE=1 \
uv run pytest tests_v2/cli/test_awiki_cli_secure_init_local.py::test_awiki_cli_msg_secure_init_command_sends_manual_direct_init -ra -q
```

Observed result:

- System-test secure init selector: 1 passed, 0 failed, 0 skipped in 2.54s.
- Failed tests: 0.
- Skipped tests: 0.
...
- The selector creates fresh e1-capable Alice/Bob test identities, seeds an
  isolated Rust CLI workspace, warms Bob's secure prekey/read path, then runs
  `msg secure init --with <bob_did>` through the real Rust subprocess.
- It verifies the public envelope: command name `awiki-cli msg secure init`,
  `meta.dry_run=false`, summary `Initialized secure session with <bob_did>`,
  `data.initialized=true`, generated `secure-init-...` message/operation IDs,
  target DID fallback, and redacted pending initiator session metadata.
- It fetches raw service-side direct history and verifies the stored message is
  `application/anp-direct-init+json` ciphertext, not plaintext or private
  session material.
- This evidence complements the existing `msg send --secure on` system flow;
  it does not cover `msg secure retry`, `msg secure repair`, runtime listener
  E2EE processing, or full secure-direct acceptance.
