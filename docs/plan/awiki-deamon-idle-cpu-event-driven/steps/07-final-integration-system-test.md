...
| 首次失败历史归类 | pytest short summary / `.pytest_cache/v/cache/lastfailed` | resolved | 首次失败曾为 CLI direct 6、CLI group 12、CLI host notify / runtime listener 7、CLI secure init/repair/retry 3、daemon message-agent 2、message-service attachment/direct/group/payload/read-watermark/sync/ws 19、user-service agent-registration 1。根因信号包括 PostgreSQL pool timeout、CLI `transport_unavailable`、cleanup 连接本地 PostgreSQL 被拒绝、registration token 缺 `controller_handle`。PostgreSQL 恢复并提交 `94a33a3` 后未复现。 |
