...
创建时间：2026-06-28  
恢复指针：Step 01、Step 02、Step 03、Step 04、Step 05 已完成；Step 06 已进入 Review 并通过验证，待创建聚焦 commit 后回填 hash。恢复时继续 Step 06 commit gate，并读取本文件、当前 Step 文档、执行台账和 `git status --short --branch`。
...
| 06 | reliable sync、gap 与 fallback 协调 | Step 05 | 串行 | 否 | coordinator | 无 | realtime supervisor、sync/fallback coordinator | WSS hint 触发 sync delta/thread-after，断线重连和低频兜底 | [steps/06-sync-gap-fallback.md](steps/06-sync-gap-fallback.md) | 必须 | reconnect/gap tests | review |
