...
创建时间：2026-06-28  
恢复指针：Step 01 和 Step 02 已完成；Step 03 已实现、验证并进入 Review/commit 收敛；恢复时从执行台账中第一个状态不是 `done` 的 Step 继续，并读取本文件、当前 Step 文档、执行台账和 `git status --short --branch`。
...
| 03 | 本地 due queue scheduler | Step 01 | A | 是 | agent-scheduler | Step 02 | queue scheduler 模块、queue state helper、局部 foreground 接入 | 四类本地队列从固定扫描改为 Notify + due timer | [steps/03-local-queue-schedulers.md](steps/03-local-queue-schedulers.md) | 必须 | scheduler tests + group daemon tests | review |
