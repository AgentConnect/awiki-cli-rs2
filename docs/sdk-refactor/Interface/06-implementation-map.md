...
本文件说明 P1 接口如何和现有 CLI 模块对应。目标是“先 façade，后搬实现”。
...
```text
im-core public API
  -> im-core internal legacy adapter
     -> current low-level implementation copied/moved from awiki-cli
```

不建议第一步这样做：

```text
CLI handler -> current low-level message/auth functions
```
...
| --- | --- | --- |
| `core.identities().list()` | CLI identity manager / identity records | 先通过 CLI adapter/legacy wrapper 读取，后移到 `im-core::identity`。 |
...
| `messages().inbox()` | current inbox | P1 返回 `Page<Message>`，允许保留 `raw` 兼容字段。 |
...
P1 中这些调用必须返回 `UnsupportedCapability`，不能静默降级：
