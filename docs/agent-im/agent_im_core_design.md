...
注意：本节是长期产品原则说明，不是 MVP 完整实现清单。MVP 只实现最小 APP action allowlist、联系人写操作确认、runtime token scope 拒绝，以及 APP 本地生成 `user_did#daemon-key-1`、user-service 只登记 / 移除 APP 提交的 public verification method 等必要边界；完整自动化能力配置面板、策略撤销 UI、审计查询/报表和复杂策略引擎放到 MVP 后版本。
...
2. APP 创建用户 DID Document 的代码路径必须先在 APP 本地生成该 key package，并从中导出 `user_did#daemon-key-1` public verification method；随后调用最新 user-service / DID API 时只提交这个 public verification method。user-service 只负责把 APP 提交的 public verification method 登记到初始 DID Document 的 `verificationMethod` 与 `authentication`，并返回包含 `#daemon-key-1` 的 DID Document。本文后续提到“初始 DID Document 带 `#daemon-key-1`”时，含义固定为：APP 本地生成 key package，user-service 只登记 public verification method；不表示 user-service 生成、派生、托管、接收或返回 daemon key/private material。bootstrap 阶段也不再追加修改 DID Document。
...
命名和所有权约束必须保持一致：MVP 只允许 `#daemon-key-1`，不得出现任何包含设备、安装环境、时间戳或用户可识别信息的 daemon key fragment；不得把“user-service 登记 public verification method”写成“user-service 生成 daemon key”；不得把 message-service MVP 授权写成依赖 user-service 管理侧状态。
...
1. `user-service` 必须支持创建 DID Document 时接收 APP 提交的 `user_did#daemon-key-1` public verification method；该 public verification method 由 APP 本地生成的 key package 导出，并且只写入 DID Document `verificationMethod` 与 `authentication`。后续撤销、轮换也以 DID Document 中该 public verification method 的替换或移除为准。user-service 不生成、不派生、不接收、不返回 daemon subkey private material。
2. `message-service` 必须支持该子 key 对普通 inbox/history 的接收权限校验；MVP 直接校验 DID proof、DID Document `authentication`、key owner 一致性和普通非 E2EE scope。MVP 运行时授权输入只有请求 proof 和当前解析到的 DID Document；message-service 不调用 user-service 读取授权状态。key 是否有效只以 DID Document `authentication` 为准。
...
MVP 需要 message-service 支持 daemon 使用 `user_did#daemon-key-1` 拉取和发送普通非 E2EE 消息。MVP 授权输入固定为请求内 DID proof、解析出的 DID Document `authentication`、key owner 一致性和普通非 E2EE scope；key 是否有效只以 DID Document `authentication` 为准。message-service 在 MVP 中不调用 user-service 读取授权状态，也不新增对 user-service 的 HTTP/RPC/内部 crate/本地缓存授权依赖。撤销对 message-service 的生效通过 DID Document `authentication` 更新和 DID Document 刷新体现。后续如需更细的本域用户级策略，可以单独设计，例如：
...
2. DID Document 管理审计（MVP 后可选）：
   ...
   - 该记录只服务 user-service 自身排障和追溯；message-service MVP 授权来源固定为 DID Document `authentication`。
...
- 需要 user-service 在 DID Document 管理 API 中支持 APP 提交的 public verification method 写入和撤销。message-service 只增加基于 DID proof、DID Document `authentication`、key owner 一致性和普通非 E2EE scope 的普通消息接收权限校验，不调用 user-service 读取授权状态。
