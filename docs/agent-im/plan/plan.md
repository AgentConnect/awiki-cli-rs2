...
| 03 | awiki-deamon bootstrap 与 user delegated identity state | Step 01、Step 02 契约 | 从普通消息发送接收 `awiki.daemon.bootstrap.v1` 明文 JSON body、明文 key package 存储、幂等状态 | [steps/03-awiki-deamon-bootstrap-state.md](steps/03-awiki-deamon-bootstrap-state.md) | 必须 | in_progress |
