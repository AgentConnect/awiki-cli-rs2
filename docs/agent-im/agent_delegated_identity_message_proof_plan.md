...
1. **第一阶段 / MVP：用户 DID 子私钥方案**
   ...
   - 创建 DID Document 的地方必须先由 APP 本地生成 key package，并从中导出 `user_did#daemon-key-1` public verification method；再调用最新 user-service / DID API，只把这个 public verification method 交给 user-service 登记 / 写入初始 DID Document 的 `verificationMethod` 与 `authentication`；user-service 不创建、不自动生成、不派生、不托管、不接收、不返回 daemon subkey private material。
...
| User Delegated Subkey / 子私钥 | APP 在创建用户 DID Document 时本地生成的一把子签名私钥，例如 `user_did#daemon-key-1`；user-service 只把对应 public verification method 登记 / 写入 DID Document 的 `verificationMethod` 与 `authentication`，不创建、不自动生成、不托管、不接收、不返回私钥。MVP 第一版 APP 通过普通消息发送明文 JSON bootstrap 传给 Daemon；后续普通消息 body 改为加密文本或加密 JSON envelope。 |
...
1. `allowed_usage_hint` 在 MVP 中只是本域策略提示，不是 DID Core 标准强制语义。运行时真正权限由 message-service 根据 DID proof、DID Document `authentication`、key owner 一致性和普通非 E2EE scope 执行；user-service 只负责 DID Document 中 public verification method 的登记 / 写入和移除；message-service MVP 不读取 user-service 侧的额外授权状态、管理记录或缓存；scoped token scope 是后续优化路径。
...
3. message-service 运行时只使用请求 proof 和当前解析到的 DID Document，不查询或依赖 user-service 侧的额外授权状态、管理记录或缓存；撤销实时性依赖 DID Document `authentication` 更新和 message-service 对 DID Document 的重新解析/刷新。
...
3. APP 调用最新 DID key management API，用新的 public verification method 替换或重新写入 #daemon-key-1。
...
2. user-service 只接收 APP 本地生成 key package 导出的 public verification method，并只把它登记 / 写入 DID Document `verificationMethod` 与 `authentication`；撤销和可选管理记录也仅针对 public verification method，不创建、不自动生成、不托管 daemon private key；
3. message-service MVP 只校验 DID proof、DID Document `authentication`、key owner 一致性和普通非 E2EE scope，不查询或依赖 user-service 侧的额外授权状态、管理记录或缓存；
...
- user-service 负责 DID Document public verification method 登记 / 写入 / revoke；message-service MVP 授权输入固定为请求 proof 和当前解析到的 DID Document，只校验 DID proof、DID Document authentication、key owner 一致性和普通消息 scope，不读取 user-service 侧的额外授权状态、管理记录或缓存。
...
2. `UserDidKeyService`
   - 在用户创建 DID Document 前由 APP 本地生成 `#daemon-key-1` private/public key package；调用最新 user-service / DID API 时，只把由该 key package 导出的 public verification method 交给 user-service 登记 / 写入 DID Document 的 `verificationMethod` 与 `authentication`；
...
1. DID Document key management API
   - 创建用户 DID Document 时接收 APP 提交的 `#daemon-key-1` public verification method；该 public verification method 由 APP 本地 key package 导出，并只登记 / 写入 DID Document `verificationMethod` 与 `authentication`；
   - user-service 不创建、不自动生成、不接收、不返回 daemon subkey private material；
   - 撤销 `#daemon-key-1` public verification method；
   - 按 DID Document 返回当前存在且在 `authentication` 中的 daemon public verification method。

2. DID Document 管理记录 API（MVP 后可选，非运行时授权输入）
   - 可记录 `user_did#daemon-key-1` public verification method 的写入、撤销、轮换和管理信息；
   - 该管理记录只服务 user-service 自身查询、撤销、排障和追溯；
   - message-service MVP 请求授权只直接校验 DID proof、DID Document `authentication`、key owner 一致性和普通非 E2EE scope，不读取 user-service 管理记录；
...
### 11.2 user-service public 管理记录表草案（MVP 后可选）

下面的表只是 user-service 管理侧可选草案，MVP 不需要实现。它只属于 public verification method 写入、撤销和追溯记录，用于 DID Document 管理侧查询、撤销和排障。message-service MVP 的运行时授权只读取 DID Document，并基于 DID proof、DID Document `authentication`、key owner 一致性和普通非 E2EE scope 判定请求；不查询或依赖这类管理记录。
...
1. APP 创建用户 DID Document 前本地生成 `#daemon-key-1` private/public key package；调用最新 DID API 时 user-service 只接收由该 key package 导出的 public verification method，并登记 / 写入 DID Document 的 `verificationMethod` 与 `authentication`。
...
11. user-service 撤销 `#daemon-key-1` 并从 DID Document `authentication` 移除后，message-service 在 DID Document 重新解析/刷新后拒绝 Daemon 拉取用户普通消息；message-service 不通过 user-service 管理记录判断 key 是否有效。
...
更准确地说，APP 必须在创建用户 DID Document 前本地生成 `#daemon-key-1` private/public key package，只把 public verification method 交给 user-service 登记 / 写入 DID Document 的 `verificationMethod` 与 `authentication`；user-service 不创建、不自动生成、不托管、不接收、不返回 daemon subkey private material。MVP bootstrap 只是把这把已经存在、且对应 public verification method 已在 DID Document `authentication` 中的子私钥通过普通消息明文 JSON 传给 Daemon。

```text
后续安全升级：仍由 APP 持有私钥所有权边界，但普通消息 body 改为加密文本或加密 JSON envelope，并把 Daemon 本地存储迁移到受保护密钥仓库。
```

因此，后续升级不默认改成“Daemon 本地生成用户 DID 子私钥”；如果未来要支持 Daemon 生成公钥再由 APP 提交给 user-service 写入 DID Document，必须作为独立方案重新评审私钥所有权、撤销和 bootstrap 信任边界。
...
user-service: 只把 APP 本地生成的 public verification method 登记 / 写入或移出 DID Document；运行时请求授权只通过 DID Document `authentication` 体现，不通过 user-service 侧额外授权状态、管理记录或缓存体现。
```
