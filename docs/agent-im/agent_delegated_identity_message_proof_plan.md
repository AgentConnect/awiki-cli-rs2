...
1. `allowed_usage_hint` 在 MVP 中只是本域策略提示，不是 DID Core 标准强制语义。运行时真正权限由 message-service 根据 DID proof、DID Document `authentication`、key owner 一致性和普通非 E2EE scope 执行；user-service 只负责 public verification method 写入和撤销；scoped token scope 是后续优化路径。
...
3. message-service 运行时只读取 DID Document，撤销实时性依赖 DID Document `authentication` 更新和 message-service 对 DID Document 的重新解析/刷新；不得调用 user-service 读取授权状态。
...
- user-service 负责 DID Document public verification method 写入 / revoke；message-service MVP 只校验 DID proof、DID Document authentication、key owner 一致性和普通消息 scope，不调用 user-service 读取授权状态。
...
2. DID Document 管理审计 API（MVP 后可选）
   ...
   - message-service MVP 请求授权只直接校验 DID proof、DID Document `authentication`、key owner 一致性和普通非 E2EE scope，不调用 user-service 读取授权状态；
...
user-service: 只把 APP 本地生成的 public verification method 写入或移出 DID Document；运行时请求授权只通过 DID Document `authentication` 体现，message-service 不调用 user-service 读取授权状态。
```
