...
1. **第一阶段 / MVP：用户 DID 子私钥方案**
   ...
   - 创建 DID Document 的地方必须先由 APP 本地生成 key package，并从中导出 `user_did#daemon-key-1` public verification method；再调用最新 user-service / DID API，只把这个 public verification method 交给 user-service 登记到初始 DID Document 的 `verificationMethod` 与 `authentication`；user-service 不创建、不默认生成、不派生、不托管、不接收、不返回 daemon subkey private material。
...
| User Delegated Subkey / 子私钥 | APP 在创建用户 DID Document 时本地生成的一把子签名私钥，例如 `user_did#daemon-key-1`；user-service 只把对应 public verification method 登记到 DID Document 的 `verificationMethod` 与 `authentication`，不创建、不默认生成、不托管、不接收、不返回私钥。MVP 第一版 APP 通过普通消息发送明文 JSON bootstrap 传给 Daemon；后续普通消息 body 改为加密文本或加密 JSON envelope。 |
...
2. user-service 只接收 APP 本地生成 key package 导出的 public verification method，并只把它登记到 DID Document `verificationMethod` 与 `authentication`；撤销和可选写入审计也仅针对 public verification method，不创建、不默认生成、不托管 daemon private key；
...
- user-service 负责 DID Document public verification method 登记 / revoke；message-service MVP 授权输入固定为请求 proof 和当前解析到的 DID Document，只校验 DID proof、DID Document authentication、key owner 一致性和普通消息 scope，不查询 user-service delegated key registry、写入审计或授权状态。
...
1. DID Document key management API
   ...
   - user-service 不创建、不默认生成、不接收、不返回 daemon subkey private material；
...
2. DID Document 写入审计 API（MVP 后可选，非运行时授权输入）
   ...
   - message-service MVP 请求授权只直接校验 DID proof、DID Document `authentication`、key owner 一致性和普通非 E2EE scope，不查询 user-service delegated key registry、写入审计或授权状态；
...
下面的表只是 user-service 侧可选写入审计草案，MVP 不需要实现。它只属于 public verification method 写入、撤销和追溯记录，用于 user-service 自身排障和追溯。它不是 delegated key registry，也不是 message-service 运行时授权输入。message-service MVP 的运行时授权只读取 DID Document，并基于 DID proof、DID Document `authentication`、key owner 一致性和普通非 E2EE scope 判定请求。
...
更准确地说，APP 必须在创建用户 DID Document 前本地生成 `#daemon-key-1` private/public key package，只把 public verification method 交给 user-service 登记到 DID Document 的 `verificationMethod` 与 `authentication`；user-service 不创建、不默认生成、不托管、不接收、不返回 daemon subkey private material。MVP bootstrap 只是把这把已经存在、且对应 public verification method 已在 DID Document `authentication` 中的子私钥通过普通消息明文 JSON 传给 Daemon。
...
user-service: 只把 APP 本地生成的 public verification method 登记到 DID Document 或从 DID Document 移出；运行时请求授权只通过 DID Document `authentication` 体现，不通过 user-service 侧 delegated key registry、写入审计或额外授权状态体现。
```
