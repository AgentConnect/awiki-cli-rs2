...
1. **第一阶段 / MVP：用户 DID 子私钥方案**
   ...
   - 创建 DID Document 的地方必须先由 APP 本地生成 key package，并从中导出 `user_did#daemon-key-1` public verification method；再调用最新 user-service / DID API，只把这个 public verification method 交给 user-service 写入初始 DID Document 的 `verificationMethod` 与 `authentication`；user-service 不创建、不生成、不派生、不托管、不接收、不返回 daemon subkey private material。
...
| User Delegated Subkey / 子私钥 | APP 在创建用户 DID Document 时本地生成的一把子签名私钥，例如 `user_did#daemon-key-1`；user-service 只把对应 public verification method 写入 DID Document 的 `verificationMethod` 与 `authentication`，不创建、不生成、不托管、不接收、不返回私钥。MVP 第一版 APP 通过普通消息发送明文 JSON bootstrap 传给 Daemon；后续普通消息 body 改为加密文本或加密 JSON envelope。 |
...
1. `allowed_usage_hint` 在 MVP 中只是本域策略提示，不是 DID Core 标准强制语义。运行时真正权限由 message-service 根据 DID proof、DID Document `authentication`、key owner 一致性和普通非 E2EE scope 执行；user-service 只负责 DID Document 中 public verification method 的写入和移除；message-service MVP 不读取 user-service 侧授权状态；scoped token scope 是后续优化路径。
...
2. user-service 只接收 APP 本地生成 key package 导出的 public verification method，并只把它写入 DID Document `verificationMethod` 与 `authentication`；撤销和可选写入审计也仅针对 public verification method，不创建、不生成、不托管 daemon private key；
...
2. `UserDidKeyService`
   - 在用户创建 DID Document 前由 APP 本地生成 `#daemon-key-1` private/public key package；调用最新 user-service / DID API 时，只把由该 key package 导出的 public verification method 交给 user-service 写入 DID Document 的 `verificationMethod` 与 `authentication`；
...
1. DID Document key management API
   ...
   - user-service 不创建、不生成、不接收、不返回 daemon subkey private material；
...
下面的表只是 user-service 侧可选写入审计草案，MVP 不需要实现。它只属于 public verification method 写入、撤销和追溯记录，用于 user-service 自身排障和追溯。message-service MVP 的运行时授权只读取 DID Document，并基于 DID proof、DID Document `authentication`、key owner 一致性和普通非 E2EE scope 判定请求。
...
1. APP 创建用户 DID Document 前本地生成 `#daemon-key-1` private/public key package；调用最新 DID API 时 user-service 只接收由该 key package 导出的 public verification method，并写入 DID Document 的 `verificationMethod` 与 `authentication`。
...
更准确地说，APP 必须在创建用户 DID Document 前本地生成 `#daemon-key-1` private/public key package，只把 public verification method 交给 user-service 写入 DID Document 的 `verificationMethod` 与 `authentication`；user-service 不创建、不生成、不托管、不接收、不返回 daemon subkey private material。MVP bootstrap 只是把这把已经存在、且对应 public verification method 已在 DID Document `authentication` 中的子私钥通过普通消息明文 JSON 传给 Daemon。
...
user-service: 只把 APP 本地生成的 public verification method 写入或移出 DID Document；运行时请求授权只通过 DID Document `authentication` 体现，不通过 user-service 侧额外授权状态体现。
```
